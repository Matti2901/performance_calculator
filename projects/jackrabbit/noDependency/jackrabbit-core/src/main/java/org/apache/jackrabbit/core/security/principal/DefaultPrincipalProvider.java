...
import org.apache.jackrabbit.core.security.util.SecuritySessionImplManager;
import org.apache.jackrabbit.core.security.util.SecuritySyncronousEventListener;
import org.apache.jackrabbit.core.security.util.UserManagerImplInterface;
 ...
 */
public class DefaultPrincipalProvider extends AbstractPrincipalProvider implements SecuritySyncronousEventListener {

    /**
     * the default logger
     */
    private static Logger log = LoggerFactory.getLogger(DefaultPrincipalProvider.class);
    private SecuritySessionImplManager ssimp;
    /**
     * Principal-Base of this Provider
     */
    private final UserManagerImplInterface userManager;
    ...
    public DefaultPrincipalProvider(Session systemSession,
                                    UserManagerImplInterface systemUserManager) throws RepositoryException {

        this.userManager = systemUserManager;
        everyonePrincipal = EveryonePrincipal.getInstance();
        ssimp = new SecuritySessionImplManager();
        String[] ntNames = new String[1];
        if (ssimp.checkSession(systemSession)) {
            NameResolver resolver = (NameResolver) systemSession;
            ntNames[0] = resolver.getJCRName(userManager.getConstantAuthorizableFolder());
            pPrincipalName = resolver.getJCRName(userManager.getP_PRINCIPAL_NAME());
        } else {
    ...
    public boolean canReadPrincipal(Session session, Principal principal) {
        ...
        // given principal
        if (ssimp.checkSession(session)) {
            
            if (ssimp.getSessionImpl(session).isAdmin() || ssimp.getSessionImpl(session).isSystem()) {
                return true;
            }
            try {
                UserManager umgr = ssimp.getSessionImpl(session).getUserManager();
