...
class EventTracker implements Event {
    ...
    public EventTracker(EventMonitor monitor, Event event) {
    ...
    private void userInfoAccessed() {
        if (!externalAccessed.get()) {
            ...
            monitor.warn("Event listener is trying to access user information of event " 
                         + event + " without checking whether the event is external.");
        }
    }

    private void dateInfoAccessed() {
        if (!externalAccessed.get()) {
            monitor.warn("Event listener is trying to access date information of event " 
                         + event + " without checking whether the event is external.");
        }
    }
