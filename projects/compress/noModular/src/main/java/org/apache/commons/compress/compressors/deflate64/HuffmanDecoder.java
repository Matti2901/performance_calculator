...
class HuffmanDecoder implements Closeable {
     ...
     */
    static final short[] RUN_LENGTH_TABLE = { 96, 128, 160, 192, 224, 256, 288, 320, 353, 417, 481, 545, 610, 738, 866, 994, 1123, 1379, 1635, 1891,
     ...
     */
    static final int[] DISTANCE_TABLE = { 16, 32, 48, 64, 81, 113, 146, 210, 275, 403, // 0-9
    ...
    static BinaryTreeNode buildTree(final int[] litTable) {
    ...
    static int nextSymbol(final BitInputStream reader, final BinaryTreeNode tree) throws IOException {
    ...
    final DecodingMemory memory = new DecodingMemory();
        ...
        while (!finalBlock || state.hasData()) {
            if (state.state() == INITIAL) {
                ...
                case 0:
                    ...
                    break;
                case 1:
                    state = new HuffmanCodes(this, FIXED_CODES, FIXED_LITERALS, FIXED_DISTANCE);
                    break;
                case 2:
                    final int[][] tables = readDynamicTables();
                    state = new HuffmanCodes(this, DYNAMIC_CODES, tables[0], tables[1]);
                    break;
                default:
    ...
    private void switchToUncompressedState() throws IOException {
        ...
        state = new UncompressedState(this, bLen);
    }
}
