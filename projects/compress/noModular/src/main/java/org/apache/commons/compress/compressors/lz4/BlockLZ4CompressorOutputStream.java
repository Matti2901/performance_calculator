...
public class BlockLZ4CompressorOutputStream extends CompressorOutputStream<OutputStream> {
    ...
    static final class Pair {
        ...
        byte[] addLiteral(final LiteralBlock block) {
        ...
        void setBackReference(final BackReference block) {
            if (hasBackReference()) {
    ...
    public BlockLZ4CompressorOutputStream(final OutputStream out, final Parameters params) {
        ...
        compressor = new LZ77Compressor(params, block -> {
            ...
            case LITERAL:
                addLiteralBlock((LiteralBlock) block);
                break;
            case BACK_REFERENCE:
                addBackReference((BackReference) block);
                break;
            case EOD:
    ...
    private void addBackReference(final BackReference block) throws IOException {
    ...
    private void addLiteralBlock(final LiteralBlock block) throws IOException {
        final Pair last = writeBlocksAndReturnUnfinishedPair(block.getLength());
    ...
    private void recordBackReference(final BackReference block) {
