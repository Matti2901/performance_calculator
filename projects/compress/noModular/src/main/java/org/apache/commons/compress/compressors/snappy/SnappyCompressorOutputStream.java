...
public class SnappyCompressorOutputStream extends CompressorOutputStream<OutputStream> {
     ...
     */
    public static Builder createParameterBuilder(final int blockSize) {
    ...
    public SnappyCompressorOutputStream(final OutputStream out, final long uncompressedSize, final Parameters params) throws IOException {
        ...
        compressor = new LZ77Compressor(params, block -> {
            ...
            case LITERAL:
                writeLiteralBlock((LiteralBlock) block);
                break;
            case BACK_REFERENCE:
                writeBackReference((BackReference) block);
                break;
            case EOD:
                break;
            }
    ...
    private void writeBackReference(final BackReference block) throws IOException {
    ...
    private void writeLiteralBlock(final LiteralBlock block) throws IOException {
        final int len = block.getLength();
    ...
    private void writeLiteralBlockFourSizeBytes(final LiteralBlock block, final int len) throws IOException {
        writeLiteralBlockWithSize(FOUR_SIZE_BYTE_MARKER, 4, len, block);
    }

    private void writeLiteralBlockNoSizeBytes(final LiteralBlock block, final int len) throws IOException {
        writeLiteralBlockWithSize(len - 1 << 2, 0, len, block);
    }

    private void writeLiteralBlockOneSizeByte(final LiteralBlock block, final int len) throws IOException {
        writeLiteralBlockWithSize(ONE_SIZE_BYTE_MARKER, 1, len, block);
    }

    private void writeLiteralBlockThreeSizeBytes(final LiteralBlock block, final int len) throws IOException {
        writeLiteralBlockWithSize(THREE_SIZE_BYTE_MARKER, 3, len, block);
    }

    private void writeLiteralBlockTwoSizeBytes(final LiteralBlock block, final int len) throws IOException {
        writeLiteralBlockWithSize(TWO_SIZE_BYTE_MARKER, 2, len, block);
    }

    private void writeLiteralBlockWithSize(final int tagByte, final int sizeBytes, final int len, final LiteralBlock block) throws IOException {
        out.write(tagByte);
