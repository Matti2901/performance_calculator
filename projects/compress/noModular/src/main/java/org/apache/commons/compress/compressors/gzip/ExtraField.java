...
public final class ExtraField implements Iterable<SubField> {
    ...
    byte[] toByteArray() {
        ...
        for (final SubField f : subFields) {
            ba[pos++] = f.si1;
            ba[pos++] = f.si2;
            ba[pos++] = (byte) (f.getPayload().length & 0xff); // little endian expected
            ba[pos++] = (byte) (f.getPayload().length >>> 8);
            System.arraycopy(f.getPayload(), 0, ba, pos, f.getPayload().length);
            pos += f.getPayload().length;
        }
        return ba;
    }
