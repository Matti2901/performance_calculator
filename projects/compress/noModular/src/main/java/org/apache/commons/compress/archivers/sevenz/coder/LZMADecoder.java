...
final class LZMADecoder extends AbstractCoder {
    ...
    @Override
    public OutputStream encode(final OutputStream out, final Object opts) throws IOException {
    ...
    @Override
    public byte[] getOptionsAsProperties(final Object opts) throws IOException {
    ...
    @Override
    public Object getOptionsFromCoder(final Coder coder, final InputStream in) throws IOException {
        if (coder.properties == null) {
            throw new IOException("Missing LZMA properties");
        }
        if (coder.properties.length < 1) {
