...
public final class FramedLZ4CompressorRoundtripTest extends AbstractTest {
    ...
    public static Stream<Arguments> factory() {
        return Stream.of(Arguments.of(new Parameters(BlockSize.K64)),
                Arguments.of(new Parameters(BlockSize.K256)),
                Arguments.of(new Parameters(BlockSize.M1)),
                Arguments.of(Parameters.DEFAULT),
                // default without content checksum
                Arguments.of(new Parameters(BlockSize.M4, false, false, false)),
                // default with block checksum
                Arguments.of(new Parameters(BlockSize.M4, true, true, false)),
                // small blocksize (so we get enough blocks) and enabled block dependency, otherwise defaults
                Arguments.of(new Parameters(BlockSize.K64, true, false, true)),
                // default, tuned for speed
                Arguments.of(new Parameters(BlockSize.M4, true, false, false,
                        BlockLZ4CompressorOutputStream.createParameterBuilder().tunedForSpeed().build())));
    }

    @ParameterizedTest
    @MethodSource("factory")
    public void biggerFileRoundtrip(final Parameters params) throws IOException {
    ...
    @ParameterizedTest
    @MethodSource("factory")
    public void blaTarRoundtrip(final Parameters params) throws IOException {
    ...
    @ParameterizedTest
    @MethodSource("factory")
    public void gzippedLoremIpsumRoundtrip(final Parameters params) throws IOException {
        roundTripTest("lorem-ipsum.txt.gz", params);
    }

    private void roundTripTest(final String testFile, final Parameters params) throws IOException {
    ...
    void test64KMultipleBlocks() throws IOException {
        ...
        try (FramedLZ4CompressorOutputStream compressor = new FramedLZ4CompressorOutputStream(buffer,
                new Parameters(BlockSize.K64, true, false, false))) {
