...
public class TestRaw
{
    ...
    public void testRaw2dealer()
    {
        ...
        SocketBase raw = ZMQ.socket(ctx, ZMQUtilConstant.ZMQ_RAW);
        assertThat(raw, notNullValue());

        boolean rc = ZMQ.setSocketOption(raw, ZMQUtilConstant.ZMQ_LINGER, 0);
        assertThat(rc, is(true));

        rc = ZMQ.bind(raw, host);
        assertThat(rc, is(true));

        host = (String) ZMQ.getSocketOptionExt(raw, ZMQUtilConstant.ZMQ_LAST_ENDPOINT);
    ...
    public void testRaw2Raw()
    {
        ...
        SocketBase bind = ZMQ.socket(ctx, ZMQUtilConstant.ZMQ_RAW);
        ...
        SocketBase connect = ZMQ.socket(ctx, ZMQUtilConstant.ZMQ_RAW);
