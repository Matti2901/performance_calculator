...
public class RepSpecTest extends AbstractSpecTest
{
    ...
    public void testSpecFairQueueIn()
    {
        ...
        for (String bindAddress : binds) {
            ...
            // strategy.
            fairQueueIn(ctx, bindAddress, ZMQUtilConstant.ZMQ_REP, ZMQUtilConstant.ZMQ_REQ);
        }
    ...
    public void testSpecEnvelope()
    {
        ...
        for (String bindAddress : binds) {
            ...
            envelope(ctx, bindAddress, ZMQUtilConstant.ZMQ_REP, ZMQUtilConstant.ZMQ_DEALER);
        }
    ...
    private void envelope(Ctx ctx, String address, int bindType, int connectType)
    {
        ...
        String host = (String) ZMQ.getSocketOptionExt(rep, ZMQUtilConstant.ZMQ_LAST_ENDPOINT);
    ...
    private void fairQueueIn(Ctx ctx, String address, int bindType, int connectType)
    {
        ...
        int timeout = 250;
        boolean rc = ZMQ.setSocketOption(rep, ZMQUtilConstant.ZMQ_RCVTIMEO, timeout);
        assertThat(rc, is(true));
        ...
        if (someoneFixThis) {
            ...
            for (int peer = 0; peer < services; ++peer) {
                ...
                String reply = new String(msg.data(), ZMQUtilConstant.CHARSET);
                replies.add(reply);
                sendSeq(rep, reply);
            }
            for (int peer = 0; peer < services; ++peer) {
