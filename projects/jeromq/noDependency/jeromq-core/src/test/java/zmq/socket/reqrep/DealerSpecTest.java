...
public class DealerSpecTest extends AbstractSpecTest
{
    ...
    public void testSpecFairQueueIn()
    {
        ...
        for (String bindAddress : binds) {
            ...
            // strategy.
            fairQueueIn(ctx, bindAddress, ZMQUtilConstant.ZMQ_DEALER, ZMQUtilConstant.ZMQ_DEALER);
        }
    ...
    public void testSpecRoundRobinOut()
    {
        ...
        for (String bindAddress : binds) {
            ...
            // strategy.
            roundRobinOut(ctx, bindAddress, ZMQUtilConstant.ZMQ_DEALER, ZMQUtilConstant.ZMQ_REP);
        }
    ...
    public void testSpecBlockOnSendNoPeers()
    {
        ...
        for (String bindAddress : binds) {
            ...
            blockOnSendNoPeers(ctx, bindAddress, ZMQUtilConstant.ZMQ_DEALER);
        }
    ...
    private void blockOnSendNoPeers(Ctx ctx, String address, int bindType)
    {
        ...
        int timeout = 250;
        boolean rc = ZMQ.setSocketOption(dealer, ZMQUtilConstant.ZMQ_SNDTIMEO, timeout);
        assertThat(rc, is(true));

        int ret = ZMQ.send(dealer, "", ZMQUtilConstant.ZMQ_DONTWAIT);
        assertThat(ret, is(-1));
        ...
        ret = ZMQ.send(dealer, "", ZMQUtilConstant.ZMQ_DONTWAIT);
        assertThat(ret, is(-1));
        assertThat(dealer.errno(), is(ZError.EAGAIN));
    ...
    private void roundRobinOut(Ctx ctx, String address, int bindType, int connectType)
    {
        ...
        for (int peer = 0; peer < services; ++peer) {
            ...
            String host = (String) ZMQ.getSocketOptionExt(dealer, ZMQUtilConstant.ZMQ_LAST_ENDPOINT);
    ...
    private void fairQueueIn(Ctx ctx, String address, int bindType, int connectType)
    {
        ...
        int timeout = 250;
        boolean rc = ZMQ.setSocketOption(receiver, ZMQUtilConstant.ZMQ_RCVTIMEO, timeout);
        assertThat(rc, is(true));
        ...
        for (int peer = 0; peer < services; ++peer) {
            ...
            rc = ZMQ.setSocketOption(sender, ZMQUtilConstant.ZMQ_RCVTIMEO, timeout);
            assertThat(rc, is(true));

            String host = (String) ZMQ.getSocketOptionExt(receiver, ZMQUtilConstant.ZMQ_LAST_ENDPOINT);
            assertThat(host, notNullValue());
