        ...
        try {
            while (true) {
                ...
                if (version != null) {
                    ...
                    String clientDomain = new String(domain.data(), ZMQUtilConstant.CHARSET);
                    ...
                    assertThat(new String(identity.data(), ZMQUtilConstant.CHARSET), is("IDENT"));
                    Msg mechanismMsg = ZMQ.recv(handler, 0);
                    assertThat("No more message",  "NULL".equals(mechanism) || mechanismMsg.hasMore());
                    String clientMechanism = new String(mechanismMsg.data(), ZMQUtilConstant.CHARSET);
                    ...
                    if ("CURVE".equals(mechanism)) {
                        ...
                        clientUsername = new String(username.data(), ZMQUtilConstant.CHARSET);
                        clientPassword = new String(password.data(), ZMQUtilConstant.CHARSET);
                    }
                    ...
                    ret = ZMQ.send(handler, authentified ? "200" : "400", ZMQUtilConstant.ZMQ_SNDMORE);
                    assertThat(ret, is(3));
                    if (authentified) {
                        ret = ZMQ.send(handler, "OK", ZMQUtilConstant.ZMQ_SNDMORE);
                        assertThat(ret, is(2));
                        ret = ZMQ.send(handler, userId, ZMQUtilConstant.ZMQ_SNDMORE);
                        assertThat(ret, is(userId.length()));
                    }
                     else {
                        ret = ZMQ.send(handler, errorMessage, ZMQUtilConstant.ZMQ_SNDMORE);
