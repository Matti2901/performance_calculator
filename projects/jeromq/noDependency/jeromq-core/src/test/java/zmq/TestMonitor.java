...
public class TestMonitor
{
    static class SocketMonitor implements Callable<AssertionError>
    {
        ...
        {
            SocketBase s = ZMQ.socket(ctx, ZMQUtilConstant.ZMQ_PAIR);
            try {
                ...
                while (true) {
                    ...
                    if (event.event == ZMQUtilConstant.ZMQ_EVENT_CONNECTED || event.event == ZMQUtilConstant.ZMQ_EVENT_ACCEPTED) {
                        activeLatch.countDown();
                    }
                    if (event.event == ZMQUtilConstant.ZMQ_EVENT_MONITOR_STOPPED) {
                    ...
                    case ZMQUtilConstant.ZMQ_EVENT_HANDSHAKE_PROTOCOL:
                    case ZMQUtilConstant.ZMQ_EVENT_CONNECT_RETRIED:
                        events |= event.event;
                        break;
                    default:
    ...
    public void testFailed()
    {
        ...
        Ctx ctx = ZMQ.init(1);
        SocketBase rep = ZMQ.socket(ctx, ZMQUtilConstant.ZMQ_REP);
        try {
    ...
    public void testMonitor() throws Exception
    {
        ...
        SocketBase rep = ZMQ.socket(ctx, ZMQUtilConstant.ZMQ_REP);
        assertThat(rep, notNullValue());
        boolean rc = ZMQ.monitorSocket(rep, "inproc://monitor.rep", ZMQUtilConstant.ZMQ_EVENT_ALL);
        assertThat(rc, is(true));
        rc = ZMQ.bind(rep, addr);
        assertThat(rc, is(true));

        // Resolve the effective listening address
        addr = (String) ZMQ.getSocketOptionExt(rep, ZMQUtilConstant.ZMQ_LAST_ENDPOINT);
        assertThat(addr, notNullValue());

        // 1st REQ socket,  monitor all events
        SocketBase req = ZMQ.socket(ctx, ZMQUtilConstant.ZMQ_REQ);
        assertThat(req, notNullValue());
        rc = ZMQ.monitorSocket(req, "inproc://monitor.req", ZMQUtilConstant.ZMQ_EVENT_ALL);
        assertThat(rc, is(true));
        rc = ZMQ.connect(req, addr);
        assertThat(rc, is(true));

        // 2nd REQ socket, monitor connected and monitor stop events only
        SocketBase req2 = ZMQ.socket(ctx, ZMQUtilConstant.ZMQ_REQ);
        assertThat(req2, notNullValue());
        rc = ZMQ.monitorSocket(req2, "inproc://monitor.req2", ZMQUtilConstant.ZMQ_EVENT_CONNECTED | ZMQUtilConstant.ZMQ_EVENT_MONITOR_STOPPED);
        assertThat(rc, is(true));
        ...
        ZMQ.setSocketOption(rep, ZMQUtilConstant.ZMQ_LINGER, 0);
        ZMQ.close(rep);

        // Close the REQ socket
        ZMQ.setSocketOption(req, ZMQUtilConstant.ZMQ_LINGER, 0);
        ZMQ.close(req);

        // Close the 2nd REQ socket
        ZMQ.setSocketOption(req2, ZMQUtilConstant.ZMQ_LINGER, 0);
        ZMQ.close(req2);
        ...
        assertTrue((threads[0].events & ZMQUtilConstant.ZMQ_EVENT_LISTENING) > 0);
        assertTrue((threads[0].events & ZMQUtilConstant.ZMQ_EVENT_ACCEPTED) > 0);
        assertTrue((threads[0].events & ZMQUtilConstant.ZMQ_EVENT_CLOSED) > 0);
        assertTrue((threads[0].events & ZMQUtilConstant.ZMQ_EVENT_MONITOR_STOPPED) > 0);
        assertTrue((threads[0].events & ZMQUtilConstant.ZMQ_EVENT_HANDSHAKE_PROTOCOL) > 0);

        // Expected REQ socket events
        assertTrue((threads[1].events & ZMQUtilConstant.ZMQ_EVENT_CONNECT_DELAYED) > 0);
        assertTrue((threads[1].events & ZMQUtilConstant.ZMQ_EVENT_CONNECTED) > 0);
        assertTrue((threads[1].events & ZMQUtilConstant.ZMQ_EVENT_MONITOR_STOPPED) > 0);
        assertTrue((threads[1].events & ZMQUtilConstant.ZMQ_EVENT_HANDSHAKE_PROTOCOL) > 0);

        // Expected 2nd REQ socket events
        assertEquals(ZMQUtilConstant.ZMQ_EVENT_CONNECTED | ZMQUtilConstant.ZMQ_EVENT_MONITOR_STOPPED, threads[2].events);
    }
    ...
    public void testDisableMonitor()
    {
        ...
        assertThat(rep, notNullValue());
        boolean rc = rep.setEventHook(e -> {
            assertThat(e.event, is(ZMQUtilConstant.ZMQ_EVENT_MONITOR_STOPPED));
            assertThat(Thread.currentThread().getName(), is("Time-limited test"));
        }, ZMQUtilConstant.ZMQ_EVENT_ALL);
        assertThat(rc, is(true));
        rc = rep.monitor(null, ZMQUtilConstant.ZMQ_EVENT_ALL);
        assertThat(rc, is(true));
        rc = ZMQ.bind(rep, addr);
        assertThat(rc, is(true));
        rep.close();
