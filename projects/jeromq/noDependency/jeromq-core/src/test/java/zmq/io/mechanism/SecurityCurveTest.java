...
public class SecurityCurveTest
{
    ...
    private Boolean runTest(boolean withzap, Function<CurveTestContext, Boolean> tested) throws InterruptedException
    {
        ...
        Runnable configurator = () -> {
            ...
            ZMQ.setSocketOption(testCtx.server, ZMQUtilConstant.ZMQ_CURVE_SERVER, true);
            ZMQ.setSocketOption(testCtx.server, ZMQUtilConstant.ZMQ_CURVE_SECRETKEY, testCtx.serverSecret);
            ZMQ.setSocketOption(testCtx.server, ZMQUtilConstant.ZMQ_IDENTITY, "IDENT");

            // Preconfigure client with valid identity, might be changed for individual tests
            ZMQ.setSocketOption(testCtx.client, ZMQUtilConstant.ZMQ_CURVE_SERVERKEY, testCtx.serverPublic);
            ZMQ.setSocketOption(testCtx.client, ZMQUtilConstant.ZMQ_CURVE_PUBLICKEY, testCtx.clientPublic);
            ZMQ.setSocketOption(testCtx.client, ZMQUtilConstant.ZMQ_CURVE_SECRETKEY, testCtx.clientSecret);
        };
    ...
    private boolean doSuccess(CurveTestContext ctx)
    {
        ...
        String host = (String) ZMQ.getSocketOptionExt(ctx.server, ZMQUtilConstant.ZMQ_LAST_ENDPOINT);
    ...
    private boolean doSuccessInverted(CurveTestContext ctx)
    {
        ...
        String host = (String) ZMQ.getSocketOptionExt(ctx.client, ZMQUtilConstant.ZMQ_LAST_ENDPOINT);
    ...
    public void testGarbageClientSecretKeyZap() throws InterruptedException
    {
        Boolean zapCheck = runTest(true, ctx -> {
            ...
            ZMQ.setSocketOption(ctx.client, ZMQUtilConstant.ZMQ_CURVE_SECRETKEY, "0000000000000000000000000000000000000000");
            String host = (String) ZMQ.getSocketOptionExt(ctx.server, ZMQUtilConstant.ZMQ_LAST_ENDPOINT);
            rc = ZMQ.connect(ctx.client, host);
    ...
    public void testGarbageServerSecretKeyZap() throws InterruptedException
    {
        Boolean zapCheck = runTest(true, ctx -> {
            ...
            ZMQ.setSocketOption(ctx.server, ZMQUtilConstant.ZMQ_CURVE_SECRETKEY, "0000000000000000000000000000000000000000");
            rc = ZMQ.bind(ctx.server, ctx.host);
            assertThat(rc, is(true));
    ...
    public void testBogusClientKey() throws InterruptedException
    {
        ...
        Boolean zapCheck = runTest(true, ctx -> {
            ...
            ZMQ.setSocketOption(ctx.client, ZMQUtilConstant.ZMQ_CURVE_PUBLICKEY, bogusPublic);
            ZMQ.setSocketOption(ctx.client, ZMQUtilConstant.ZMQ_CURVE_SECRETKEY, bogusSecret);
            String host = (String) ZMQ.getSocketOptionExt(ctx.server, ZMQUtilConstant.ZMQ_LAST_ENDPOINT);
            rc = ZMQ.connect(ctx.client, host);
            assertThat(rc, is(true));
            return false;
    ...
    public void testNullClient() throws InterruptedException
    {
        ...
        Boolean zapCheck = runTest(false, ctx -> {
            ...
            ctx.client = ZMQ.socket(ctx.zctxt, ZMQUtilConstant.ZMQ_DEALER);
    ...
    public void testPlainClient() throws InterruptedException
    {
        ...
        Boolean zapCheck = runTest(false, ctx -> {
            ...
            ZMQ.closeZeroLinger(ctx.client);
            ctx.client = ZMQ.socket(ctx.zctxt, ZMQUtilConstant.ZMQ_DEALER);
            ZMQ.setSocketOption(ctx.client, ZMQUtilConstant.ZMQ_PLAIN_USERNAME, "user");
            ZMQ.setSocketOption(ctx.client, ZMQUtilConstant.ZMQ_PLAIN_PASSWORD, "pass");
    ...
    public void inconsistent1()
    {
        MechanismTester.checkOptions(Mechanisms.CURVE, opt -> {
            opt.setSocketOpt(ZMQUtilConstant.ZMQ_CURVE_PUBLICKEY, new byte[32]);
            opt.setSocketOpt(ZMQUtilConstant.ZMQ_CURVE_SECRETKEY, null);
        });
    }
    ...
    {
        MechanismTester.checkOptions(Mechanisms.CURVE, opt -> {
            opt.setSocketOpt(ZMQUtilConstant.ZMQ_CURVE_PUBLICKEY, null);
            opt.setSocketOpt(ZMQUtilConstant.ZMQ_CURVE_SECRETKEY, new byte[32]);
        });
    }
    ...
    public void inconsistent3()
    {
        MechanismTester.checkOptions(Mechanisms.CURVE, opt -> {
            opt.setSocketOpt(ZMQUtilConstant.ZMQ_CURVE_PUBLICKEY, new byte[32]);
            opt.setSocketOpt(ZMQUtilConstant.ZMQ_CURVE_SECRETKEY, new byte[31]);
        });
    }
    ...
    {
        MechanismTester.checkOptions(Mechanisms.CURVE, opt -> {
            opt.setSocketOpt(ZMQUtilConstant.ZMQ_CURVE_PUBLICKEY, new byte[31]);
            opt.setSocketOpt(ZMQUtilConstant.ZMQ_CURVE_SECRETKEY, new byte[32]);
        });
    }
    ...
    public void inconsistent5()
    {
        MechanismTester.checkOptions(Mechanisms.CURVE, opt -> {
            opt.setSocketOpt(ZMQUtilConstant.ZMQ_CURVE_PUBLICKEY, new byte[32]);
            opt.setSocketOpt(ZMQUtilConstant.ZMQ_CURVE_SECRETKEY, new byte[32]);
            opt.setSocketOpt(ZMQUtilConstant.ZMQ_CURVE_SERVERKEY, new byte[31]);
        });
    }
    ...
    public void consistent2()
    {
        MechanismTester.checkOptions(Mechanisms.CURVE, opt -> {
            opt.setSocketOpt(ZMQUtilConstant.ZMQ_CURVE_PUBLICKEY, new byte[32]);
            opt.setSocketOpt(ZMQUtilConstant.ZMQ_CURVE_SECRETKEY, new byte[32]);
            opt.setSocketOpt(ZMQUtilConstant.ZMQ_CURVE_SERVERKEY, new byte[32]);
        });
    }
}
