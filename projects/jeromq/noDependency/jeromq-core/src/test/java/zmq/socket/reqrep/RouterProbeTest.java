...
public class RouterProbeTest
{
    ...
    public void testProbeRouter() throws IOException
    {
        ...
        SocketBase server = ZMQ.socket(ctx, ZMQUtilConstant.ZMQ_ROUTER);
        ...
        SocketBase client = ZMQ.socket(ctx, ZMQUtilConstant.ZMQ_ROUTER);
        assertThat(client, notNullValue());

        rc = ZMQ.setSocketOption(client, ZMQUtilConstant.ZMQ_IDENTITY, "X");
        assertThat(rc, is(true));

        rc = ZMQ.setSocketOption(client, ZMQUtilConstant.ZMQ_PROBE_ROUTER, true);
        assertThat(rc, is(true));
        ...
        int ret = ZMQ.send(server, "X", ZMQUtilConstant.ZMQ_SNDMORE);
        assertThat(ret, is(1));
        ...
        msg = ZMQ.recv(client, 0);
        assertThat(msg, notNullValue());
        assertThat(new String(msg.data(), ZMQUtilConstant.CHARSET), is("Hello"));
