...
public class TestSocketPoll
{
    ...
    public void testPollIn() throws Exception
    {
        ...
        // Create a server
        SocketBase server = ZMQ.socket(context, ZMQUtilConstant.ZMQ_SERVER);
        ...
        // create a client
        SocketBase client = ZMQ.socket(context, ZMQUtilConstant.ZMQ_CLIENT);
        assertThat(client, notNullValue());
        rc = ZMQ.connect(client, "tcp://localhost:" + port);
        assertThat(rc, is(true));

        //  poll no message
        int ready = server.poll(ZMQUtilConstant.ZMQ_POLLIN, 10, null);
        ...
        ready = server.poll(ZMQUtilConstant.ZMQ_POLLIN, 10, null);
        assertThat(ready, is(ZMQUtilConstant.ZMQ_POLLIN));
    ...
    public void cancelPoll() throws Exception
    {
        ...
        //  poll canceled
        int ready = server.poll(ZMQUtilConstant.ZMQ_POLLIN, -1, cancellationToken);
        assertThat(ready, is(-1));
    ...
    public void testZeroTimeout() throws Exception
    {
        ...
        //  poll zero timeout
        int ready = server.poll(ZMQUtilConstant.ZMQ_POLLIN, 0, null);
        assertThat(ready, is(-1));
        assertThat(server.errno(), is(ZError.EAGAIN));
