...
class MechanismTester
{
            ...
            Runnable configurator) throws InterruptedException
    {
        ...
        if (withzap) {
            testCtx.zapHandler = ZMQ.socket(testCtx.zctxt, ZMQUtilConstant.ZMQ_REP);
        ...
        testCtx.server = ZMQ.socket(testCtx.zctxt, ZMQUtilConstant.ZMQ_DEALER);
        assertThat(testCtx.server, notNullValue());

        testCtx.client = ZMQ.socket(testCtx.zctxt, ZMQUtilConstant.ZMQ_DEALER);
    ...
    public static <T extends TestContext> boolean testRawSocket(T ctx)
    {
        try {
            ...
            int timeout = 250;
            ZMQ.setSocketOption(ctx.server, ZMQUtilConstant.ZMQ_RCVTIMEO, timeout);

            rc = ZMQ.bind(ctx.server, ctx.host);
            assertThat(rc, is(true));
            int port = TestUtils.port((String) ZMQ.getSocketOptionExt(ctx.server, ZMQUtilConstant.ZMQ_LAST_ENDPOINT));
            ...
            try (Socket sock = new Socket("127.0.0.1", port)) {
                ...
                out.write(("1" + 0x00).getBytes(ZMQUtilConstant.CHARSET));
                // send sneaky message that shouldn't be received
                out.write(
                        ("8" + 0x00 + "sneaky" + 0x00)
                                .getBytes(ZMQUtilConstant.CHARSET));
