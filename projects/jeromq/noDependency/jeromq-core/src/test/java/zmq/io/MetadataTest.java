...
public class MetadataTest
{
    private static class ZapHandler implements Runnable
    {
        ...
        public void run()
        {
            ...
            while (true) {
                ...
                assertThat(new String(version.data(), ZMQUtilConstant.CHARSET), is("1.0"));
                assertThat(new String(mechanism.data(), ZMQUtilConstant.CHARSET), is("NULL"));

                int ret = ZMQ.send(handler, version, ZMQUtilConstant.ZMQ_SNDMORE);
                assertThat(ret, is(3));
                ret = ZMQ.send(handler, sequence, ZMQUtilConstant.ZMQ_SNDMORE);
                assertThat(ret, is(1));

                System.out.println("Sending ZAP reply");
                if ("DOMAIN".equals(new String(domain.data(), ZMQUtilConstant.CHARSET))) {
                    ret = ZMQ.send(handler, "200", ZMQUtilConstant.ZMQ_SNDMORE);
                    assertThat(ret, is(3));
                    ret = ZMQ.send(handler, "OK", ZMQUtilConstant.ZMQ_SNDMORE);
                    assertThat(ret, is(2));
                    ret = ZMQ.send(handler, "anonymous", ZMQUtilConstant.ZMQ_SNDMORE);
                    assertThat(ret, is(9));
                    ret = ZMQ.send(handler, metadata, metadata.length, 0);
                    assertThat(ret, is(metadata.length));
                }
                else {
                    ret = ZMQ.send(handler, "400", ZMQUtilConstant.ZMQ_SNDMORE);
                    assertThat(ret, is(3));
                    ret = ZMQ.send(handler, "BAD DOMAIN", ZMQUtilConstant.ZMQ_SNDMORE);
                    assertThat(ret, is(10));
                    ret = ZMQ.send(handler, "", ZMQUtilConstant.ZMQ_SNDMORE);
                    assertThat(ret, is(0));
    ...
    public void testMetadata() throws IOException, InterruptedException
    {
        ...
        SocketBase handler = ZMQ.socket(ctx, ZMQUtilConstant.ZMQ_REP);
        ...
        SocketBase server = ZMQ.socket(ctx, ZMQUtilConstant.ZMQ_DEALER);
        assertThat(server, notNullValue());
        SocketBase client = ZMQ.socket(ctx, ZMQUtilConstant.ZMQ_DEALER);
        assertThat(client, notNullValue());

        ZMQ.setSocketOption(server, ZMQUtilConstant.ZMQ_ZAP_DOMAIN, "DOMAIN");

        ZMQ.setSocketOption(server, ZMQUtilConstant.ZMQ_SELFADDR_PROPERTY_NAME, "X-Local-Address");
