...
public class ConnectRidTest
{
    ...
    public void testStream2stream()
    {
        ...
        Msg msg = new Msg("hi 1".getBytes(ZMQUtilConstant.CHARSET));

        Ctx ctx = ZMQ.init(1);
        assert (ctx != null);

        //  Set up listener STREAM.
        SocketBase bind = ZMQ.socket(ctx, ZMQUtilConstant.ZMQ_STREAM);
        assert (bind != null);

        ZMQ.setSocketOption(bind, ZMQUtilConstant.ZMQ_CONNECT_RID, "connectRid");
        ZMQ.setSocketOption(bind, ZMQUtilConstant.ZMQ_LINGER, 0);

        boolean rc = ZMQ.bind(bind, host);
        assert (rc);

        host = (String) ZMQ.getSocketOptionExt(bind, ZMQUtilConstant.ZMQ_LAST_ENDPOINT);
        assertThat(host, notNullValue());

        //  Set up connection stream.
        SocketBase connect = ZMQ.socket(ctx, ZMQUtilConstant.ZMQ_STREAM);
        assert (connect != null);

        ZMQ.setSocketOption(connect, ZMQUtilConstant.ZMQ_LINGER, 0);

        //  Do the connection.
        ZMQ.setSocketOption(connect, ZMQUtilConstant.ZMQ_CONNECT_RID, "connectRid");
        ...
        */
        ...
        int ret = ZMQ.send(connect, "connectRid", ZMQUtilConstant.ZMQ_SNDMORE);
        assert (10 == ret);
    ...
    private void testRouter2router(boolean named)
    {
        ...
        //  Set up listener STREAM.
        SocketBase bind = ZMQ.socket(ctx, ZMQUtilConstant.ZMQ_ROUTER);
        assert (bind != null);
        ...
        //  Set up connection stream.
        SocketBase connect = ZMQ.socket(ctx, ZMQUtilConstant.ZMQ_ROUTER);
        assert (connect != null);
        ...
        if (named) {
            ZMQ.setSocketOption(bind, ZMQUtilConstant.ZMQ_IDENTITY, "X");
            ZMQ.setSocketOption(connect, ZMQUtilConstant.ZMQ_IDENTITY, "Y");
        }
        ...
        */
        ...
        //  Send some data back.
        if (named) {
            ret = ZMQ.send(bind, name, ZMQUtilConstant.ZMQ_SNDMORE);
            assertThat(ret, is(1));
        }
        else {
            ret = ZMQ.send(bind, name, ZMQUtilConstant.ZMQ_SNDMORE);
    ...
    public void testRouter2routerWhileReceiving()
    {
        ...
        SocketBase x = ZMQ.socket(ctx, ZMQUtilConstant.ZMQ_ROUTER);
        assert (x != null);

        ZMQ.setSocketOption(x, ZMQUtilConstant.ZMQ_LINGER, 0);
        ZMQ.setSocketOption(x, ZMQUtilConstant.ZMQ_IDENTITY, "X");

        boolean rc = ZMQ.bind(x, wildcardAddress);
        assert (rc);

        String xaddress = (String) ZMQ.getSocketOptionExt(x, ZMQUtilConstant.ZMQ_LAST_ENDPOINT);
        assertThat(xaddress, notNullValue());

        //  Set up the router which binds
        SocketBase z = ZMQ.socket(ctx, ZMQUtilConstant.ZMQ_ROUTER);
        assert (z != null);

        ZMQ.setSocketOption(z, ZMQUtilConstant.ZMQ_LINGER, 0);
        ZMQ.setSocketOption(z, ZMQUtilConstant.ZMQ_IDENTITY, "Z");

        rc = ZMQ.bind(z, wildcardAddress);
        assert (rc);

        String zaddress = (String) ZMQ.getSocketOptionExt(z, ZMQUtilConstant.ZMQ_LAST_ENDPOINT);
        assertThat(zaddress, notNullValue());

        //  Set up connect-only router.
        SocketBase y = ZMQ.socket(ctx, ZMQUtilConstant.ZMQ_ROUTER);
        assert (y != null);

        ZMQ.setSocketOption(y, ZMQUtilConstant.ZMQ_LINGER, 0);
        ZMQ.setSocketOption(y, ZMQUtilConstant.ZMQ_IDENTITY, "Y");
        ...
        int ret = ZMQ.send(y, "X", ZMQUtilConstant.ZMQ_SNDMORE);
        assertThat(ret, is(1));
        ...
        ZMQ.setSocketOption(x, ZMQUtilConstant.ZMQ_CONNECT_RID, "Z");
        rc = ZMQ.connect(x, zaddress);
        assert (rc);

        //  Send data from X to Z
        ret = ZMQ.send(x, "Z", ZMQUtilConstant.ZMQ_SNDMORE);
        assertThat(ret, is(1));
        ...
        Msg name = ZMQ.recv(y, ZMQUtilConstant.ZMQ_DONTWAIT);
