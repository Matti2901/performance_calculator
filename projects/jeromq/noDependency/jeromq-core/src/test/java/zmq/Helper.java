...
public class Helper
{
    ...
    {
        byte[] content = "12345678ABCDEFGH12345678abcdefgh".getBytes(ZMQUtilConstant.CHARSET);

        //  Send the message.
        int rc = ZMQ.send(sc, content, 32, ZMQUtilConstant.ZMQ_SNDMORE);
        assert (rc == 32);
        ...
        msg = ZMQ.recv(sb, 0);
        assert (msg.size() == 32);
        long rcvmore = ZMQ.getSocketOption(sb, ZMQUtilConstant.ZMQ_RCVMORE);
        assert (rcvmore == 1);
        msg = ZMQ.recv(sb, 0);
        assert (rc == 32);
        rcvmore = ZMQ.getSocketOption(sb, ZMQUtilConstant.ZMQ_RCVMORE);
        assert (rcvmore == 0);
        rc = ZMQ.send(sb, new Msg(msg), ZMQUtilConstant.ZMQ_SNDMORE);
        assert (rc == 32);
        ...
        msg = ZMQ.recv(sc, 0);
        assert (rc == 32);
        rcvmore = ZMQ.getSocketOption(sc, ZMQUtilConstant.ZMQ_RCVMORE);
    ...
    {
        final byte[] content = "12345678ABCDEFGH12345678abcdefgh".getBytes(ZMQUtilConstant.CHARSET);
        final int timeout = 250;
        final Errno errno = new Errno();

        //  Send message from client to server
        ZMQ.setSocketOption(client, ZMQUtilConstant.ZMQ_SNDTIMEO, timeout);

        int rc = ZMQ.send(client, content, 32, ZMQUtilConstant.ZMQ_SNDMORE);
        assert ((rc == 32) || ((rc == -1) && errno.is(ZError.EAGAIN)));
        rc = ZMQ.send(client, content, 32, 0);
        assert ((rc == 32) || ((rc == -1) && errno.is(ZError.EAGAIN)));

        //  Receive message at server side (should not succeed)
        ZMQ.setSocketOption(server, ZMQUtilConstant.ZMQ_RCVTIMEO, timeout);
        ...
        ZMQ.setSocketOption(server, ZMQUtilConstant.ZMQ_SNDTIMEO, timeout);

        rc = ZMQ.send(server, content, 32, ZMQUtilConstant.ZMQ_SNDMORE);
        assert (rc == 32 || ((rc == -1) && errno.is(ZError.EAGAIN)));
        rc = ZMQ.send(server, content, 32, 0);
        assert (rc == 32 || ((rc == -1) && errno.is(ZError.EAGAIN)));

        //  Receive message at client side (should not succeed)
        ZMQ.setSocketOption(client, ZMQUtilConstant.ZMQ_RCVTIMEO, timeout);
    ...
    {
        return ZMQ.send(socket, data, ZMQUtilConstant.ZMQ_SNDMORE);
    }
    ...
    public static String recv(SocketBase socket)
    {
        ...
        assert (msg != null);
        return new String(msg.data(), ZMQUtilConstant.CHARSET);
    }
    ...
    {
        byte[] content = data.getBytes(ZMQUtilConstant.CHARSET);

        byte[] length = String.format("%04d", content.length).getBytes(ZMQUtilConstant.CHARSET);
        ...
        reslen = Integer.parseInt(new String(buf, 0, 4, ZMQUtilConstant.CHARSET));

        in.read(buf, 0, reslen);
        System.out.println("recv " + reslen + " " + new String(buf, 0, reslen, ZMQUtilConstant.CHARSET));
    }
}
