...
public class StreamTest
{
    ...
    public void testStream2dealer()
    {
        ...
        SocketBase stream = ZMQ.socket(ctx, ZMQUtilConstant.ZMQ_STREAM);
        assertThat(stream, notNullValue());

        boolean rc = ZMQ.setSocketOption(stream, ZMQUtilConstant.ZMQ_LINGER, 0);
        assertThat(rc, is(true));

        rc = ZMQ.bind(stream, host);
        assertThat(rc, is(true));

        host = (String) ZMQ.getSocketOptionExt(stream, ZMQUtilConstant.ZMQ_LAST_ENDPOINT);
        assertThat(host, notNullValue());

        //  We'll be using this socket as the other peer
        SocketBase dealer = ZMQ.socket(ctx, ZMQUtilConstant.ZMQ_DEALER);
        assertThat(dealer, notNullValue());
        rc = ZMQ.setSocketOption(dealer, ZMQUtilConstant.ZMQ_LINGER, 0);
        assertThat(rc, is(true));
        ...
        ret = ZMQ.send(stream, id, ZMQUtilConstant.ZMQ_SNDMORE);
        ...
        //  Send Ready command
        ZMQ.send(stream, id, ZMQUtilConstant.ZMQ_SNDMORE);
        ZMQ.send(
                 stream,
        ...
        assertThat(recv.data(), is("World".getBytes(ZMQUtilConstant.CHARSET)));
