...
public class TestTimeo
{
    static class Worker implements Runnable
    {
        ...
        {
            ZMQ.sleep(1);
            SocketBase sc = ZMQ.socket(ctx, ZMQUtilConstant.ZMQ_PUSH);
    ...
    public void testTimeo() throws Exception
    {
        ...
        SocketBase sb = ZMQ.socket(ctx, ZMQUtilConstant.ZMQ_PULL);
        assertThat(sb, notNullValue());
        boolean rc = ZMQ.bind(sb, "inproc://timeout_test");
        assertThat(rc, is(true));

        //  Check whether non-blocking recv returns immediately.
        Msg msg = ZMQ.recv(sb, ZMQUtilConstant.ZMQ_DONTWAIT);
        assertThat(msg, nullValue());

        //  Check whether recv timeout is honoured.
        int timeout = 500;
        ZMQ.setSocketOption(sb, ZMQUtilConstant.ZMQ_RCVTIMEO, timeout);
        ...
        assertThat(sc, notNullValue());
        ZMQ.setSocketOption(sb, ZMQUtilConstant.ZMQ_RCVTIMEO, timeout);
        ZMQ.setSocketOption(sb, ZMQUtilConstant.ZMQ_SNDTIMEO, timeout);
        rc = ZMQ.connect(sc, "inproc://timeout_test");
        assertThat(rc, is(true));
        Msg smsg = new Msg("12345678ABCDEFGH12345678abcdefgh".getBytes(ZMQUtilConstant.CHARSET));
