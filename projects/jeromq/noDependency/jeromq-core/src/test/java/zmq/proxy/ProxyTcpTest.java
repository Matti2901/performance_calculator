...
public class ProxyTcpTest
{
    ...
    static class Dealer extends Thread
    {
        ...
        {
            this.s = ZMQ.socket(ctx, ZMQUtilConstant.ZMQ_DEALER);
            this.name = name;
            this.port = port;
        }
        ...
        public void run()
        {
            ...
            while (true) {
                ...
                String data = new String(msg.data(), 0, msg.size(), ZMQUtilConstant.CHARSET);

                Msg response;
                if ((i % 3) == 2) {
                    response = new Msg(msg.size() + 3);
                    response.put("OK ".getBytes(ZMQUtilConstant.CHARSET)).put(msg.data());
                }
                else {
                    response = new Msg(msg.data());
                }

                s.send(response, (i % 3) == 2 ? 0 : ZMQUtilConstant.ZMQ_SNDMORE);
                i++;
                if (data.equals("end")) {
                    break;
                }
    ...
    public static class ProxyDecoder extends Decoder
    {
        ...
        {
            size = Integer.parseInt(new String(header, ZMQUtilConstant.CHARSET));
        ...
        {
            System.out.println("Received body " + new String(msg.data(), ZMQUtilConstant.CHARSET));
    ...
    public static class ProxyEncoder extends EncoderBase
    {
        ...
        private void writeHeader()
        {
            ...
            header.clear();
            header.put(String.format("%04d", msg.size()).getBytes(ZMQUtilConstant.CHARSET));
            header.flip();
    ...
    static class Proxy extends Thread
    {
        ...
        {
            boolean rc;
            SocketBase routerBind = ZMQ.socket(ctx, ZMQUtilConstant.ZMQ_ROUTER);
            assertThat(routerBind, notNullValue());

            routerBind.setSocketOpt(ZMQUtilConstant.ZMQ_DECODER, ProxyDecoder.class);
            routerBind.setSocketOpt(ZMQUtilConstant.ZMQ_ENCODER, ProxyEncoder.class);

            rc = ZMQ.bind(routerBind, "tcp://127.0.0.1:" + routerPort);
            assertThat(rc, is(true));

            SocketBase dealerBind = ZMQ.socket(ctx, ZMQUtilConstant.ZMQ_DEALER);
