...
public class ImmediateTest
{
    ...
    public void testImmediateTrue() throws Exception
    {
        ...
        SocketBase to = ZMQ.socket(context, ZMQUtilConstant.ZMQ_PULL);
        assertThat(to, notNullValue());

        int val = 0;
        boolean rc = ZMQ.setSocketOption(to, ZMQUtilConstant.ZMQ_LINGER, val);
        assertThat(rc, is(true));
        rc = ZMQ.bind(to, "tcp://*:" + pushPort1);
        assertThat(rc, is(true));

        // Create a socket pushing to two endpoints - only 1 message should arrive.
        SocketBase from = ZMQ.socket(context, ZMQUtilConstant.ZMQ_PUSH);
        assertThat(from, notNullValue());

        rc = ZMQ.setSocketOption(from, ZMQUtilConstant.ZMQ_IMMEDIATE, true);
        assertThat(rc, is(true));

        val = 0;
        rc = ZMQ.setSocketOption(from, ZMQUtilConstant.ZMQ_LINGER, val);
        assertThat(rc, is(true));
        ...
        int timeout = 250;
        ZMQ.setSocketOption(to, ZMQUtilConstant.ZMQ_RCVTIMEO, timeout);
    ...
    public void testImmediateFalse() throws Exception
    {
        ...
        int val = 0;
        rc = ZMQ.setSocketOption(to, ZMQUtilConstant.ZMQ_LINGER, val);
        assertThat(rc, is(true));
        ...
        // Set the key flag
        rc = ZMQ.setSocketOption(from, ZMQUtilConstant.ZMQ_IMMEDIATE, false);
        assertThat(rc, is(true));
    ...
    public void testImmediateFalseWithBrokenConnection() throws Exception
    {
        ...
        SocketBase backend = ZMQ.socket(context, ZMQUtilConstant.ZMQ_DEALER);
        assertThat(backend, notNullValue());

        SocketBase frontend = ZMQ.socket(context, ZMQUtilConstant.ZMQ_DEALER);
        assertThat(frontend, notNullValue());

        final int linger = 0;
        ZMQ.setSocketOption(backend, ZMQUtilConstant.ZMQ_LINGER, linger);
        ZMQ.setSocketOption(frontend, ZMQUtilConstant.ZMQ_LINGER, linger);

        //  Frontend connects to backend using IMMEDIATE
        ZMQ.setSocketOption(frontend, ZMQUtilConstant.ZMQ_IMMEDIATE, false);
        ...
        sent = ZMQ.send(frontend, "Hello", ZMQUtilConstant.ZMQ_DONTWAIT);
        assertThat(sent, is(5));
        ...
        while (ZMQ.send(frontend, "Hello", ZMQUtilConstant.ZMQ_DONTWAIT) != -1) {
            ZMQ.sleep(2);
        }

        //  Recreate backend socket
        backend = ZMQ.socket(context, ZMQUtilConstant.ZMQ_DEALER);
        ZMQ.setSocketOption(backend, ZMQUtilConstant.ZMQ_LINGER, linger);
