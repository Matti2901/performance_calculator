...
public class PushPullSpecTest extends AbstractSpecTest
{
    ...
    public void testSpecPullFairQueueIn()
    {
        ...
        for (String bindAddress : binds) {
            ...
            // strategy.
            fairQueueIn(ctx, bindAddress, ZMQUtilConstant.ZMQ_PULL, ZMQUtilConstant.ZMQ_PUSH);
        }
    ...
    public void testSpecPushRoundRobinOut()
    {
        ...
        for (String bindAddress : binds) {
            ...
            roundRobinOut(ctx, bindAddress, ZMQUtilConstant.ZMQ_PUSH, ZMQUtilConstant.ZMQ_PULL);
        }
    ...
    public void testSpecPushBlockOnSendNoPeers()
    {
        ...
        for (String bindAddress : binds) {
            ...
            // available peers.
            blockOnSendNoPeers(ctx, bindAddress, ZMQUtilConstant.ZMQ_PUSH);
        }
    ...
    private void blockOnSendNoPeers(Ctx ctx, String address, int bindType)
    {
        ...
        int timeout = 250;
        boolean rc = ZMQ.setSocketOption(push, ZMQUtilConstant.ZMQ_SNDTIMEO, timeout);
        assertThat(rc, is(true));

        int ret = ZMQ.send(push, "", ZMQUtilConstant.ZMQ_DONTWAIT);
        assertThat(ret, is(-1));
        ...
        ret = ZMQ.send(push, "", ZMQUtilConstant.ZMQ_DONTWAIT);
        assertThat(ret, is(-1));
        assertThat(push.errno(), is(ZError.EAGAIN));
    ...
    private void roundRobinOut(Ctx ctx, String address, int bindType, int connectType)
    {
        ...
        for (int peer = 0; peer < services; ++peer) {
            ...
            String host = (String) ZMQ.getSocketOptionExt(push, ZMQUtilConstant.ZMQ_LAST_ENDPOINT);
    ...
    private void fairQueueIn(Ctx ctx, String address, int bindType, int connectType)
    {
        ...
        for (int peer = 0; peer < services; ++peer) {
            ...
            String host = (String) ZMQ.getSocketOptionExt(pull, ZMQUtilConstant.ZMQ_LAST_ENDPOINT);
            assertThat(host, notNullValue());
        ...
        for (int peer = 0; peer < services; ++peer) {
            ...
            firstHalf.remove(new String(msg.data(), ZMQUtilConstant.CHARSET));
        }
        ...
        for (int peer = 0; peer < services; ++peer) {
            ...
            assertThat(msg, notNullValue());
            assertThat(msg.size(), is(2));
            secondHalf.remove(new String(msg.data(), ZMQUtilConstant.CHARSET));
        }
