...
public class TestInvalidRep
{
    ...
    public void testInvalidRep()
    {
        ...
        SocketBase routerSocket = ZMQ.socket(ctx, ZMQUtilConstant.ZMQ_ROUTER);
        assertThat(routerSocket, notNullValue());

        SocketBase reqSocket = ZMQ.socket(ctx, ZMQUtilConstant.ZMQ_REQ);
        assertThat(reqSocket, notNullValue());
        int linger = 0;
        int rc;
        ZMQ.setSocketOption(routerSocket, ZMQUtilConstant.ZMQ_LINGER, linger);
        ZMQ.setSocketOption(reqSocket, ZMQUtilConstant.ZMQ_LINGER, linger);
        ...
        //  Send valid reply.
        rc = ZMQ.send(routerSocket, addr, ZMQUtilConstant.ZMQ_SNDMORE);
        assertThat(rc, is(addrSize));
        rc = ZMQ.send(routerSocket, bottom, ZMQUtilConstant.ZMQ_SNDMORE);
        assertThat(rc, is(0));
