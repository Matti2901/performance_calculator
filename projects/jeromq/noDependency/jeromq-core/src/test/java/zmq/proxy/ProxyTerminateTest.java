...
public class ProxyTerminateTest
{
    private static class ServerTask implements Runnable
    {
        ...
        public void run()
        {
            ...
            try {
                frontend = ZMQ.socket(ctx, ZMQUtilConstant.ZMQ_SUB);
                assertThat(frontend, notNullValue());
                ZMQ.setSocketOption(frontend, ZMQUtilConstant.ZMQ_SUBSCRIBE, "");
                boolean rc = ZMQ.bind(frontend, hostFrontend);
                assertThat(rc, is(true));
                backend = ZMQ.socket(ctx, ZMQUtilConstant.ZMQ_PUSH);
                assertThat(backend, notNullValue());
                rc = ZMQ.bind(frontend, hostBackend);
                assertThat(rc, is(true));
                control = ZMQ.socket(ctx, ZMQUtilConstant.ZMQ_SUB);
    ...
    public void testProxyTerminate() throws IOException, InterruptedException, ExecutionException
    {
        ...
        SocketBase publisher = ZMQ.socket(ctx, ZMQUtilConstant.ZMQ_PUB);
