...
public abstract class AbstractProtocolVersion
{
            ...
            throws IOException, InterruptedException
    {
        ...
        SocketBase receiver = ZMQ.socket(ctx, ZMQUtilConstant.ZMQ_PULL);
        assertThat(receiver, notNullValue());

        boolean rc = ZMQ.setSocketOption(receiver, ZMQUtilConstant.ZMQ_LINGER, 0);
        assertThat(rc, is(true));

        rc = ZMQ.monitorSocket(receiver, "inproc://monitor", ZMQUtilConstant.ZMQ_EVENT_HANDSHAKE_PROTOCOL);
        assertThat(rc, is(true));
        ...
        String ep = (String) ZMQ.getSocketOptionExt(receiver, ZMQUtilConstant.ZMQ_LAST_ENDPOINT);
        ...
        assertThat(new String(msg.data(), ZMQUtilConstant.CHARSET), is(payload));

        monitor.join();

        final Event event = monitor.events[0];
        assertThat(event, notNullValue());
        assertThat(event.event, is(ZMQUtilConstant.ZMQ_EVENT_HANDSHAKE_PROTOCOL));
    ...
    protected List<ByteBuffer> raws(int revision)
    {
        ...
        // socket type
        raw.put((byte) ZMQUtilConstant.ZMQ_PUSH);
