...
public class ProxyTest
{
    private static final class Client implements Callable<Boolean>
    {
        ...
        public Boolean call()
        {
            ...
            SocketBase control = ZMQ.socket(ctx, ZMQUtilConstant.ZMQ_SUB);
            assertThat(control, notNullValue());

            boolean rc = ZMQ.setSocketOption(control, ZMQUtilConstant.ZMQ_SUBSCRIBE, ZMQUtilConstant.SUBSCRIPTION_ALL);
            ...
            rc = ZMQ.setSocketOption(client, ZMQUtilConstant.ZMQ_IDENTITY, identity);
            assertThat(rc, is(true));
            ...
            items[0] = new PollItem(client, ZMQUtilConstant.ZMQ_POLLIN);
            items[1] = new PollItem(control, ZMQUtilConstant.ZMQ_POLLIN);
            ...
            while (run && ! Thread.currentThread().isInterrupted()) {
                ...
                for (int centitick = 0; centitick < 20; centitick++) {
                    int count = ZMQPoll.poll(selector, items, 10);
                    if (items[0].isReadable()) {
                        count--;
                        Msg msgrsp = ZMQ.recv(client, 0);

                        String payload = new String(msgrsp.data(), ZMQUtilConstant.CHARSET);
                        if (verbose) {
                        ...
                        int more = ZMQ.getSocketOption(client, ZMQUtilConstant.ZMQ_RCVMORE);
                        assertThat(more, is(0));
                    }
                    if (items[1].isReadable()) {
                        count--;
                        Msg msgin = ZMQ.recv(control, 0);
                        if (Arrays.equals(msgin.data(), ZMQUtilConstant.PROXY_TERMINATE)) {
                            run = false;
                            break;
                        }
                    }
                ...
                Msg msgout = new Msg(payload.getBytes(ZMQUtilConstant.CHARSET));
    ...
    private static final class Server implements Callable<Boolean>
    {
        ...
        public Boolean call() throws InterruptedException, ExecutionException, TimeoutException
        {
            ...
            SocketBase frontend = ZMQ.socket(ctx, ZMQUtilConstant.ZMQ_ROUTER);
            ...
            SocketBase backend = ZMQ.socket(ctx, ZMQUtilConstant.ZMQ_DEALER);
            ...
            rc = ZMQ.setSocketOption(control, ZMQUtilConstant.ZMQ_SUBSCRIBE, ZMQUtilConstant.SUBSCRIPTION_ALL);
            assertThat(rc, is(true));
    ...
    private static final class Worker implements Callable<Boolean>
    {
        ...
        public Boolean call()
        {
            SocketBase worker = ZMQ.socket(ctx, ZMQUtilConstant.ZMQ_DEALER);
            ...
            rc = ZMQ.setSocketOption(control, ZMQUtilConstant.ZMQ_SUBSCRIBE, new byte[0]);
            assertThat(rc, is(true));
            ...
            started.countDown();
            while (run) {
                msg = ZMQ.recv(control, ZMQUtilConstant.ZMQ_DONTWAIT);
                if (control.errno() == ZError.ETERM) {
                    break;
                }
                if (msg != null) {
                    if (Arrays.equals(msg.data(), ZMQUtilConstant.PROXY_TERMINATE)) {
                        break;
                    }
                }
                ...
                Msg identity = ZMQ.recv(worker, ZMQUtilConstant.ZMQ_DONTWAIT);
                if (identity != null) {
                    ...
                    for (int idx = 0; idx < random.nextInt(5); ++idx) {
                        ...
                        int sent = ZMQ.send(worker, identity, ZMQUtilConstant.ZMQ_SNDMORE);
    ...
    public void testProxy() throws Throwable
    {
        ...
        // Control socket receives terminate command from main over inproc
        SocketBase control = ZMQ.socket(ctx, ZMQUtilConstant.ZMQ_PUB);
        assertThat(control, notNullValue());
        ...
        while (true) {
            Thread.sleep(100);
            int sent = ZMQ.send(control, ZMQUtilConstant.PROXY_TERMINATE, 0);
