...
public class ReqSpecTest extends AbstractSpecTest
{
    ...
    public void testSpecMessageFormat()
    {
        ...
        for (String bindAddress : binds) {
            ...
            //   application.
            messageFormat(ctx, bindAddress, ZMQUtilConstant.ZMQ_REQ, ZMQUtilConstant.ZMQ_ROUTER);
        }
    ...
    public void testSpecRoundRobinOut()
    {
        ...
        for (String bindAddress : binds) {
            ...
            // strategy.
            roundRobinOut(ctx, bindAddress, ZMQUtilConstant.ZMQ_REQ, ZMQUtilConstant.ZMQ_REP);
        }
    ...
    public void testSpecBlockOnSendNoPeers()
    {
        ...
        for (String bindAddress : binds) {
            ...
            // connected peers.
            blockOnSendNoPeers(ctx, bindAddress, ZMQUtilConstant.ZMQ_REQ);
        }
    ...
    public void testSpecOnlyListensToCurrentPeer()
    {
        ...
        for (String bindAddress : binds) {
            ...
            // complete.
            onlyListensToCurrentPeer(ctx, bindAddress, ZMQUtilConstant.ZMQ_REQ, ZMQUtilConstant.ZMQ_ROUTER);
        }
    ...
    private void onlyListensToCurrentPeer(Ctx ctx, String bindAddress, int bindType, int connectType)
    {
        ...
        boolean rc = ZMQ.setSocketOption(socket, ZMQUtilConstant.ZMQ_IDENTITY, "A");
        ...
        for (int peer = 0; peer < services; ++peer) {
            ...
            rc = ZMQ.setSocketOption(connect, ZMQUtilConstant.ZMQ_RCVTIMEO, timeout);
            assertThat(rc, is(true));

            rc = ZMQ.setSocketOption(connect, ZMQUtilConstant.ZMQ_ROUTER_MANDATORY, true);
            assertThat(rc, is(true));

            String host = (String) ZMQ.getSocketOptionExt(socket, ZMQUtilConstant.ZMQ_LAST_ENDPOINT);
    ...
    private void blockOnSendNoPeers(Ctx ctx, String address, int bindType)
    {
        ...
        int timeout = 250;
        boolean rc = ZMQ.setSocketOption(socket, ZMQUtilConstant.ZMQ_SNDTIMEO, timeout);
        assertThat(rc, is(true));

        int ret = ZMQ.send(socket, "", ZMQUtilConstant.ZMQ_DONTWAIT);
        assertThat(ret, is(-1));
        ...
        ret = ZMQ.send(socket, "", ZMQUtilConstant.ZMQ_DONTWAIT);
        assertThat(ret, is(-1));
        assertThat(socket.errno(), is(ZError.EAGAIN));
    ...
    private void roundRobinOut(Ctx ctx, String address, int bindType, int connectType)
    {
        ...
        for (int peer = 0; peer < services; ++peer) {
            ...
            rc = ZMQ.setSocketOption(reps, ZMQUtilConstant.ZMQ_RCVTIMEO, timeout);
            assertThat(rc, is(true));

            String host = (String) ZMQ.getSocketOptionExt(req, ZMQUtilConstant.ZMQ_LAST_ENDPOINT);
            assertThat(host, notNullValue());
    ...
    private void messageFormat(Ctx ctx, String address, int bindType, int connectType)
    {
        ...
        int more = ZMQ.getSocketOption(router, ZMQUtilConstant.ZMQ_RCVMORE);
        ...
        int ret = ZMQ.send(router, msg, ZMQUtilConstant.ZMQ_SNDMORE);
