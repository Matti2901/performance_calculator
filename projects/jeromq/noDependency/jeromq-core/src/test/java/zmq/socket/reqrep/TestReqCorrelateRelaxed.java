...
public class TestReqCorrelateRelaxed
{
    ...
    public void overallSetup()
    {
        ...
        SocketBase reqClient = ZMQ.socket(ctx, ZMQUtilConstant.ZMQ_REQ);
        assertThat(reqClient, notNullValue());

        reqClient.setSocketOpt(ZMQUtilConstant.ZMQ_REQ_CORRELATE, 1);
        reqClient.setSocketOpt(ZMQUtilConstant.ZMQ_REQ_RELAXED, 1);
        reqClient.setSocketOpt(ZMQUtilConstant.ZMQ_RCVTIMEO, 100);
    ...
    public byte[] testReqSentFrames(SocketBase dealer, SocketBase reqClient)
    {
        ...
        assertThat(receivedReqId.flags() & ZMQUtilConstant.ZMQ_MORE, not(0));
        ...
        assertThat(receivedEmpty.flags() & ZMQUtilConstant.ZMQ_MORE, not(0));
        ...
        assertThat(receivedPayload.flags() & ZMQUtilConstant.ZMQ_MORE, is(0));
    ...
    public void testReqRecvGoodRequestId(SocketBase dealer, SocketBase reqClient, byte[] origRequestId)
    {
        ...
        // Send response
        assertThat(ZMQ.send(dealer, requestId, ZMQUtilConstant.ZMQ_SNDMORE), is(REQUEST_ID_LENGTH));
        assertThat(ZMQ.send(dealer, empty, ZMQUtilConstant.ZMQ_SNDMORE), is(0));
    ...
    public void testReqRecvBadRequestId(SocketBase dealer, SocketBase reqClient, byte[] origRequestId)
    {
        ...
        assertThat(ZMQ.send(dealer, badRequestId, ZMQUtilConstant.ZMQ_SNDMORE), is(12));
        assertThat(ZMQ.send(dealer, empty, ZMQUtilConstant.ZMQ_SNDMORE), is(0));
        assertThat(ZMQ.send(dealer, badResponsePayload, 0), is(badResponsePayload.size()));

        // Send response with good request ID
        assertThat(ZMQ.send(dealer, goodRequestId, ZMQUtilConstant.ZMQ_SNDMORE), is(REQUEST_ID_LENGTH));
        assertThat(ZMQ.send(dealer, empty, ZMQUtilConstant.ZMQ_SNDMORE), is(0));
