...
public class CustomDecoderTest
{
    static class CustomDecoder extends Decoder
    {
        ...
        private Step.Result readHeader()
        {
            ...
            assertThat(new String(headerBuff, 0, 6, ZMQUtilConstant.CHARSET), is("HEADER"));
    ...
    {
        in.put("1234567890".getBytes(ZMQUtilConstant.CHARSET));
        in.put("1234567890".getBytes(ZMQUtilConstant.CHARSET));
    }

    private int readHeader(ByteBuffer in)
    {
        in.put("HEADER".getBytes(ZMQUtilConstant.CHARSET));
        in.putInt(20);
        return in.position();
    }
    ...
    public void testAssignCustomDecoder()
    {
        ...
        boolean rc = socket.setSocketOpt(ZMQUtilConstant.ZMQ_DECODER, CustomDecoder.class);
    ...
    public void testAssignWrongCustomDecoder()
    {
        ...
        try {
            socket.setSocketOpt(ZMQUtilConstant.ZMQ_DECODER, WrongDecoder.class);
        }
        finally {
            ZMQ.close(socket);
            ZMQ.term(ctx);
