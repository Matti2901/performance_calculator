...
public class ZMsg implements Iterable<ZFrame>, Deque<ZFrame>
{
     ...
     */
    public boolean send(ISocket socket)
    {
     ...
     * @return true if send is success, false otherwise
     */
    public boolean send(ISocket socket, boolean destroy)
    {
        if (socket == null) {
     ...
     */
    public static ZMsg recvMsg(ISocket socket)
    {
     ...
     *          ZMsg object, null if interrupted
     */
    public static ZMsg recvMsg(ISocket socket, boolean wait)
    {
        return recvMsg(socket, wait ? 0 : ZMQUtilConstant.ZMQ_DONTWAIT);
    }
     ...
     *          ZMsg object, null if interrupted
     */
    public static ZMsg recvMsg(ISocket socket, int flag)
    {
        if (socket == null) {
            throw new IllegalArgumentException("socket is null");
        }
     ...
     */
    @Draft
    public static void recvMsg(ISocket socket, int flags,
     ...
     */
    @Draft
    public static void recvMsg(ISocket socket, int flags, Consumer<ZMsg> handler)
    {
        handler.accept(ZMsg.recvMsg(socket, flags));
    }
