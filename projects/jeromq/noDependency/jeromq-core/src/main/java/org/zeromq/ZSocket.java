...
public class ZSocket implements AutoCloseable
{
    ...
    public int getType()
    {
        return (int) getOption(ZMQUtilConstant.ZMQ_TYPE);
    }
    ...
    {
        return (int) getOption(ZMQUtilConstant.ZMQ_RCVMORE) == 1;
    }

    public void subscribe(byte[] topic)
    {
        setOption(ZMQUtilConstant.ZMQ_SUBSCRIBE, topic);
    }

    public void subscribe(String topic)
    {
        setOption(ZMQUtilConstant.ZMQ_SUBSCRIBE, topic.getBytes(ZMQUtilConstant.CHARSET));
    }

    public void unsubscribe(byte[] topic)
    {
        setOption(ZMQUtilConstant.ZMQ_UNSUBSCRIBE, topic);
    }

    public void unsubscribe(String topic)
    {
        setOption(ZMQUtilConstant.ZMQ_UNSUBSCRIBE, topic.getBytes(ZMQUtilConstant.CHARSET));
    }
    ...
    public boolean sendMessage(ZMsg message)
    {
        ...
        boolean rc = false;
        while (frame != null) {
            rc = sendFrame(frame, !message.isEmpty() ? ZMQUtilConstant.ZMQ_MORE : 0);
            if (!rc) {
                break;
            }
            frame = message.pop();
    ...
    {
        final byte[] b = str.getBytes(ZMQUtilConstant.CHARSET);
        return send(b, flags);
    }
    ...
    public String receiveStringUtf8(int flags)
    {
        ...
        return new String(b, ZMQUtilConstant.CHARSET);
    }
