...
public class ZFrame
{
    public static final int MORE     = ZMQUtilConstant.ZMQ_SNDMORE;
    ...
    {
        if (data != null) {
            this.data = data.getBytes(ZMQUtilConstant.CHARSET);
        }
    }
     ...
     */
    public boolean send(ISocket socket, int flags)
    {
        ...
        int sendFlags = (flags & ZFrame.MORE) == ZFrame.MORE ? ZMQUtilConstant.ZMQ_SNDMORE : 0;
        sendFlags |= (flags & ZFrame.DONTWAIT) == ZFrame.DONTWAIT ? ZMQUtilConstant.ZMQ_DONTWAIT : 0;
     ...
     *          True if success, else False
     */
    public boolean sendAndKeep(ISocket socket, int flags)
    {
     ...
     *          True if success, else False
     */
    public boolean sendAndKeep(ISocket socket)
    {
     ...
     *          True if success, else False
     */
    public boolean sendAndDestroy(ISocket socket, int flags)
    {
     ...
     *          True if success, else False
     */
    public boolean sendAndDestroy(ISocket socket)
    {
     ...
     */
    public static ZFrame recvFrame(ISocket socket)
    {
     ...
     *              received frame, else null
     */
    public static ZFrame recvFrame(ISocket socket, int flags)
    {
        final SocketBase base = socket.base();
