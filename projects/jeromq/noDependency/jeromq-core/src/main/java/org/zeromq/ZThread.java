...
public class ZThread
{
    ...
    {
        void run(Object[] args, IZContext ctx, Socket pipe);
    }
    ...
    {
        private IZContext          ctx;
        private IAttachedRunnable attachedRunnable;
        private IDetachedRunnable detachedRunnable;
        private final Object[]    args;
        private Socket            pipe;

        protected ShimThread(IZContext ctx, IAttachedRunnable runnable, Object[] args, Socket pipe)
        {
            assert (ctx != null);
            assert (pipe != null);
        ...
        public void run()
        {
            if (attachedRunnable != null) {
                try {
                    ...
                    if (e.getErrorCode() != ZError.Error.ETERM.getCode()) {
                        throw e;
                    }
                }
                ctx.destroy();
    ...
    public static Socket fork(IZContext ctx, IAttachedRunnable runnable, Object... args)
    {
