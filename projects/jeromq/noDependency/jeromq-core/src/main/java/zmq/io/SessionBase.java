...
public class SessionBase extends Own implements Pipe.IPipeEvents, IPollEvents
{
    ...
    public int zapConnect()
    {
        ...
        if (zapPipe == null) {
            ...
            if (peer.options.type != ZMQUtilConstant.ZMQ_REP && peer.options.type != ZMQUtilConstant.ZMQ_ROUTER &&
                        peer.options.type != ZMQUtilConstant.ZMQ_SERVER) {
                errno.set(ZError.ECONNREFUSED);
                return ZError.ECONNREFUSED;
            }
    ...
    protected void processAttach(IEngine engine)
    {
        ...
        if (pipe == null && !isTerminating()) {
            ...
            boolean conflate = options.conflate && (options.type == ZMQUtilConstant.ZMQ_DEALER || options.type == ZMQUtilConstant.ZMQ_PULL
                    || options.type == ZMQUtilConstant.ZMQ_PUSH || options.type == ZMQUtilConstant.ZMQ_PUB || options.type == ZMQUtilConstant.ZMQ_SUB);
    ...
    private void reconnect()
    {
        ...
        if (pipe != null && (options.type == ZMQUtilConstant.ZMQ_SUB || options.type == ZMQUtilConstant.ZMQ_XSUB)) {
            pipe.hiccup();
        }
    }
