 ...
 * the library.<br/>
 * It creates a reaper thread and some IO threads as defined by {@link ZMQUtilConstant#ZMQ_IO_THREADS}. The thread are created
...
public class Ctx implements ICtx
{
    ...
    public Ctx()
    {
        ...
        slotCount = 0;
        slots = null;
        maxSockets = ZMQUtilConstant.ZMQ_MAX_SOCKETS_DFLT;
        ioThreadCount = ZMQUtilConstant.ZMQ_IO_THREADS_DFLT;
    ...
    public void terminate()
    {
        ...
        try {
            ...
            for (Entry<PendingConnection, String> pending : pendingConnections.entries()) {
                SocketBase s = createSocket(ZMQUtilConstant.ZMQ_PAIR);
   ...
   /**
    ...
    {
        if (option == ZMQUtilConstant.ZMQ_MAX_SOCKETS && optval >= 1) {
            chechStarted();
            optSync.lock();
            try {
                maxSockets = optval;
        ...
        else if (option == ZMQUtilConstant.ZMQ_IO_THREADS && optval >= 0) {
            chechStarted();
            optSync.lock();
            try {
        ...
        else if (option == ZMQUtilConstant.ZMQ_BLOCKY && optval >= 0) {
            optSync.lock();
            try {
                blocky = (optval != 0);
            }
        ...
        else if (option == ZMQUtilConstant.ZMQ_IPV6 && optval >= 0) {
            optSync.lock();
            try {
                ipv6 = (optval != 0);
            }
    ...
    {
        int rc;
        if (option == ZMQUtilConstant.ZMQ_MAX_SOCKETS) {
            rc = maxSockets;
        }
        else if (option == ZMQUtilConstant.ZMQ_IO_THREADS) {
            rc = ioThreadCount;
        }
        else if (option == ZMQUtilConstant.ZMQ_BLOCKY) {
            rc = blocky ? 1 : 0;
        }
        else if (option == ZMQUtilConstant.ZMQ_IPV6) {
            rc = ipv6 ? 1 : 0;
        }
        else {
                                      ...
                                      Side side)
    {
        ...
        boolean conflate = pendingConnection.endpoint.options.conflate
                && (pendingConnection.endpoint.options.type == ZMQUtilConstant.ZMQ_DEALER
                        || pendingConnection.endpoint.options.type == ZMQUtilConstant.ZMQ_PULL
                        || pendingConnection.endpoint.options.type == ZMQUtilConstant.ZMQ_PUSH
                        || pendingConnection.endpoint.options.type == ZMQUtilConstant.ZMQ_PUB
                        || pendingConnection.endpoint.options.type == ZMQUtilConstant.ZMQ_SUB);
