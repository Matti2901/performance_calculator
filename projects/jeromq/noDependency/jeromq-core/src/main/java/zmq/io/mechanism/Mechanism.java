...
public abstract class Mechanism
{
    ...
    private void setUserId(byte[] data)
    {
        ...
        zapProperties.put(Metadata.USER_ID, new String(data, ZMQUtilConstant.CHARSET));
    }
    ...
    {
        addProperty(buf, name, value.getBytes(ZMQUtilConstant.CHARSET));
    }

    protected final void addProperty(Msg msg, String name, String value)
    {
        addProperty(msg, name, value.getBytes(ZMQUtilConstant.CHARSET));
    }

    protected final void addProperty(ByteBuffer buf, String name, byte[] value)
    {
        byte[] nameB = name.getBytes(ZMQUtilConstant.CHARSET);
    ...
    protected int parseErrorMessage(Msg msg)
    {
        if (msg.size() < 7 && msg.size() != 6) {
            session.getSocket().eventHandshakeFailedProtocol(session.getEndpoint(), ZMQUtilConstant.ZMQ_PROTOCOL_ERROR_ZMTP_MALFORMED_COMMAND_ERROR);
        ...
        if (msg.size() >= 7) {
            ...
            if (msg.size() == 10) {
                ...
                String statusCode = new String(statusBuffer, ZMQUtilConstant.CHARSET);
    ...
    protected int handleErrorReason(String reason)
    {
        ...
        if (reason.length() == 3
            ...
            try {
                ...
                session.getSocket().eventHandshakeFailedProtocol(session.getEndpoint(), ZMQUtilConstant.ZMQ_PROTOCOL_ERROR_ZAP_MALFORMED_REPLY);
            }
        }
        else {
            session.getSocket().eventHandshakeFailedProtocol(session.getEndpoint(), ZMQUtilConstant.ZMQ_PROTOCOL_ERROR_ZAP_MALFORMED_REPLY);
    ...
    protected final void sendZapRequest(Mechanisms mechanism, boolean more)
    {
        ...
        msg = new Msg(3);
        msg.setFlags(Msg.MORE);
        msg.put("1.0".getBytes(ZMQUtilConstant.CHARSET));
        ...
        msg = new Msg(1);
        msg.setFlags(Msg.MORE);
        msg.put("1".getBytes(ZMQUtilConstant.CHARSET));
        rc = session.writeZapMsg(msg);
        assert (rc);
        ...
        msg.setFlags(Msg.MORE);
        msg.put(options.zapDomain.getBytes(ZMQUtilConstant.CHARSET));
        rc = session.writeZapMsg(msg);
        assert (rc);

        //  Address frame
        byte[] host = peerAddress.host().getBytes(ZMQUtilConstant.CHARSET);
        ...
        msg.put(mechanism.name().getBytes(ZMQUtilConstant.CHARSET));
        if (more) {
            msg.setFlags(Msg.MORE);
        }
        rc = session.writeZapMsg(msg);
    ...
    protected final int receiveAndProcessZapReply()
    {
        ...
        if (msgs.get(0).size() > 0) {
            session.getSocket().eventHandshakeFailedProtocol(session.getEndpoint(), ZMQUtilConstant.ZMQ_PROTOCOL_ERROR_ZAP_UNSPECIFIED);
            return ZError.EPROTO;
        }

        //  Version frame
        if (msgs.get(1).size() != 3 || !compare(msgs.get(1), "1.0", false)) {
            session.getSocket().eventHandshakeFailedProtocol(session.getEndpoint(), ZMQUtilConstant.ZMQ_PROTOCOL_ERROR_ZAP_BAD_VERSION);
            return ZError.EPROTO;
        }

        //  Request id frame
        if (msgs.get(2).size() != 1 || !compare(msgs.get(2), "1", false)) {
            session.getSocket().eventHandshakeFailedProtocol(session.getEndpoint(), ZMQUtilConstant.ZMQ_PROTOCOL_ERROR_ZAP_BAD_REQUEST_ID);
            return ZError.EPROTO;
        }

        //  Status code frame
        if (msgs.get(3).size() != 3) {
            session.getSocket().eventHandshakeFailedProtocol(session.getEndpoint(), ZMQUtilConstant.ZMQ_PROTOCOL_ERROR_ZAP_INVALID_STATUS_CODE);
            return ZError.EPROTO;
        }

        //  Save status code
        statusCode = new String(msgs.get(3).data(), ZMQUtilConstant.CHARSET);
