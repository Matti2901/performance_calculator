...
class Proxy
{
    ...
    private boolean start(SocketBase frontend, SocketBase backend, SocketBase capture, SocketBase control)
    {
        ...
        items[0] = new PollItem(frontend, ZMQUtilConstant.ZMQ_POLLIN);
        items[1] = new PollItem(backend, ZMQUtilConstant.ZMQ_POLLIN);
        if (control != null) {
            items[2] = new PollItem(control, ZMQUtilConstant.ZMQ_POLLIN);
        }
        PollItem[] itemsout = new PollItem[2];

        itemsout[0] = new PollItem(frontend, ZMQUtilConstant.ZMQ_POLLOUT);
        itemsout[1] = new PollItem(backend, ZMQUtilConstant.ZMQ_POLLOUT);
        ...
        try {
            while (state != State.TERMINATED) {
                ...
                rc = ZMQPoll.poll(selector, items, -1);
                if (rc < 0) {
                    return false;
                }
                ...
                if (frontend != backend) {
                    rc = ZMQPoll.poll(selector, itemsout, 0L);
                    if (rc < 0) {
                        return false;
                    }
                }
                ...
                if (control != null && items[2].isReadable()) {
                    ...
                    more = control.getSocketOpt(ZMQUtilConstant.ZMQ_RCVMORE);
                    ...
                    if (Arrays.equals(command, ZMQUtilConstant.PROXY_PAUSE)) {
                        state = State.PAUSED;
                    }
                    else if (Arrays.equals(command, ZMQUtilConstant.PROXY_RESUME)) {
                        state = State.ACTIVE;
                    }
                    else if (Arrays.equals(command, ZMQUtilConstant.PROXY_TERMINATE)) {
                        state = State.TERMINATED;
                    }
                    else {
                        //  This is an API error, we should assert
                        System.out
                                .printf("E: invalid command sent to proxy '%s'%n", new String(command, ZMQUtilConstant.CHARSET));
                        assert false;
                    }
                }
                //  Process a request.
    ...
    private boolean forward(SocketBase from, SocketBase to, SocketBase capture)
    {
        ...
        while (true) {
            ...
            more = from.getSocketOpt(ZMQUtilConstant.ZMQ_RCVMORE);
            if (more < 0) {
                return false;
            }
            ...
            success = to.send(msg, more > 0 ? ZMQUtilConstant.ZMQ_SNDMORE : 0);
            if (!success) {
                return false;
            }
            if (more == 0) {
    ...
    private boolean capture(SocketBase capture, Msg msg, int more)
    {
        if (capture != null) {
            ...
            return capture.send(ctrl, more > 0 ? ZMQUtilConstant.ZMQ_SNDMORE : 0);
        }
        return true;
    }
}
