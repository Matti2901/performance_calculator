...
public class XPub extends SocketBase implements  IXPub
{
    private static final class SendUnsubscription implements Mtrie.IMtrieHandler
    {
        @Override
        public void invoke(Pipe pipe, byte[] data, int size, IXPub self)
        {
    ...
    public XPub(Ctx parent, int tid, int sid)
    {
        ...
        options.type = ZMQUtilConstant.ZMQ_XPUB;
        verboseSubs = false;
        verboseUnsubs = false;
        more = false;
        lossy = true;
    ...
    protected void xreadActivated(Pipe pipe)
    {
        ...
        while ((sub = pipe.read()) != null) {
            ...
            if (manual) {
                ...
                if (options.type == ZMQUtilConstant.ZMQ_XPUB && notify) {
                    pendingData.add(Blob.createBlob(sub));
                    pendingFlags.add(0);
                }
            }
    ...
    {
        if (option == ZMQUtilConstant.ZMQ_XPUB_VERBOSE || option == ZMQUtilConstant.ZMQ_XPUB_VERBOSER
                || option == ZMQUtilConstant.ZMQ_XPUB_NODROP || option == ZMQUtilConstant.ZMQ_XPUB_MANUAL) {
            if (option == ZMQUtilConstant.ZMQ_XPUB_VERBOSE) {
                verboseSubs = Options.parseBoolean(option, optval);
                verboseUnsubs = false;
            }
            else if (option == ZMQUtilConstant.ZMQ_XPUB_VERBOSER) {
                verboseSubs = Options.parseBoolean(option, optval);
                verboseUnsubs = verboseSubs;
            }
            else if (option == ZMQUtilConstant.ZMQ_XPUB_NODROP) {
                lossy = !Options.parseBoolean(option, optval);
            }
            else if (option == ZMQUtilConstant.ZMQ_XPUB_MANUAL) {
                manual = Options.parseBoolean(option, optval);
            }
        }
        else if (option == ZMQUtilConstant.ZMQ_SUBSCRIBE && manual) {
            if (null != lastPipe) {
                String val = Options.parseString(option, optval);
                subscriptions.add(new Msg(val.getBytes()), lastPipe);
            }
        }
        else if (option == ZMQUtilConstant.ZMQ_UNSUBSCRIBE && manual) {
            if (null != lastPipe) {
                String val = Options.parseString(option, optval);
    ...
    public void sendUnsubscription(byte[] data, int size)
    {
        if (options.type != ZMQUtilConstant.ZMQ_PUB) {
