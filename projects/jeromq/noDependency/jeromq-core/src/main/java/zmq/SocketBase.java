...
import zmq.socket.reqrep.Sockets;
...
public abstract class SocketBase extends Own implements IPollEvents, Pipe.IPipeEvents
{
    ...
    protected SocketBase(Ctx parent, int tid, int sid, boolean threadSafe)
    {
        ...
        options.ipv6 = parent.get(ZMQUtilConstant.ZMQ_IPV6) != 0;
        options.linger = parent.get(ZMQUtilConstant.ZMQ_BLOCKY) != 0 ? -1 : 0;
    ...
    public final int getSocketOpt(int option)
    {
        ...
        try {
            ...
            if (option == ZMQUtilConstant.ZMQ_RCVMORE) {
                return rcvmore ? 1 : 0;
            }
            if (option == ZMQUtilConstant.ZMQ_EVENTS) {
                ...
                int val = 0;
                if (hasOut()) {
                    val |= ZMQUtilConstant.ZMQ_POLLOUT;
                }
                if (hasIn()) {
                    val |= ZMQUtilConstant.ZMQ_POLLIN;
                }
                return val;
            }
    ...
    public final Object getSocketOptx(int option)
    {
        ...
        if (option == ZMQUtilConstant.ZMQ_FD) {
            if (threadSafe) {
    ...
    public final int connectPeer(String addr)
    {
        ...
        try {
            if (options.type != ZMQUtilConstant.ZMQ_PEER && options.type != ZMQUtilConstant.ZMQ_RAW) {
    ...
    private boolean connectInternal(String addr)
    {
        ...
        if (protocol == NetProtocol.inproc) {
            ...
            boolean conflate = options.conflate && (options.type == ZMQUtilConstant.ZMQ_DEALER || options.type == ZMQUtilConstant.ZMQ_PULL
                                                    || options.type == ZMQUtilConstant.ZMQ_PUSH || options.type == ZMQUtilConstant.ZMQ_PUB
                                                    || options.type == ZMQUtilConstant.ZMQ_SUB);
        ...
        boolean isSingleConnect = options.type == ZMQUtilConstant.ZMQ_DEALER || options.type == ZMQUtilConstant.ZMQ_SUB
                                  || options.type == ZMQUtilConstant.ZMQ_REQ;
    ...
    public final boolean send(Msg msg, int flags, AtomicBoolean canceled)
    {
        ...
        try {
            ...
            if ((flags & ZMQUtilConstant.ZMQ_SNDMORE) > 0) {
            ...
            if ((flags & ZMQUtilConstant.ZMQ_DONTWAIT) > 0 || options.sendTimeout == 0) {
                return false;
            }
    ...
    public final Msg recv(int flags, AtomicBoolean canceled)
    {
        ...
        try {
            ...
            if ((flags & ZMQUtilConstant.ZMQ_DONTWAIT) > 0 || options.recvTimeout == 0) {
                if (!processCommands(0, false, canceled)) {
                    return null;
                }
                ticks = 0;
    ...
    public final int poll(int interest, int timeout, AtomicBoolean canceled)
    {
        ...
        try {
            ...
            if (hasOut() && (interest & ZMQUtilConstant.ZMQ_POLLOUT) > 0) {
                ready |= ZMQUtilConstant.ZMQ_POLLOUT;
            }

            if (hasIn() && (interest & ZMQUtilConstant.ZMQ_POLLIN) > 0) {
    ...
    public final boolean monitor(final String addr, int events)
    {
        synchronized (monitor) {
            ...
            SocketBase monitorSocket = getCtx().createSocket(ZMQUtilConstant.ZMQ_PAIR);
            if (monitorSocket == null) {
                return false;
            }
            // Never block context termination on pending event messages
            monitorSocket.setSocketOpt(ZMQUtilConstant.ZMQ_LINGER, 0);
    ...
    private void stopMonitor()
    {
        ...
        if (monitor.get() != null) {
            if ((monitorEvents & ZMQUtilConstant.ZMQ_EVENT_MONITOR_STOPPED) != 0) {
                monitorEvent(new ZMQ.Event(ZMQUtilConstant.ZMQ_EVENT_MONITOR_STOPPED, "", 0));
            }
            monitor.get().close();
            monitor.set(null);
            monitorEvents = 0;
