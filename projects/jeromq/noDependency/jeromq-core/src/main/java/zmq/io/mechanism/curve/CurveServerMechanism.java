...
public class CurveServerMechanism extends Mechanism
{
    ...
    public int processHandshakeCommand(Msg msg)
    {
        ...
        case EXPECT_INITIATE:
            ...
            break;
        default:
            session.getSocket().eventHandshakeFailedProtocol(session.getEndpoint(), ZMQUtilConstant.ZMQ_PROTOCOL_ERROR_ZMTP_UNSPECIFIED);
            rc = ZError.EPROTO;
            break;
    ...
    public Msg encode(Msg msg)
    {
        ...
        messageNonce.put("CurveZMQMESSAGES".getBytes(ZMQUtilConstant.CHARSET));
    ...
    public Msg decode(Msg msg)
    {
        ...
        if (!compare(msg, "MESSAGE", true)) {
            session.getSocket().eventHandshakeFailedProtocol(session.getEndpoint(), ZMQUtilConstant.ZMQ_PROTOCOL_ERROR_ZMTP_UNEXPECTED_COMMAND);
            errno.set(ZError.EPROTO);
            return null;
        }

        if (msg.size() < 33) {
            session.getSocket().eventHandshakeFailedProtocol(session.getEndpoint(), ZMQUtilConstant.ZMQ_PROTOCOL_ERROR_ZMTP_MALFORMED_COMMAND_MESSAGE);
            errno.set(ZError.EPROTO);
            return null;
        }

        ByteBuffer messageNonce = ByteBuffer.allocate(Curve.Size.NONCE.bytes());
        messageNonce.put("CurveZMQMESSAGEC".getBytes(ZMQUtilConstant.CHARSET));
        msg.transfer(messageNonce, 8, 8);

        long nonce = msg.getLong(8);

        if (nonce <= cnPeerNonce) {
            session.getSocket().eventHandshakeFailedProtocol(session.getEndpoint(), ZMQUtilConstant.ZMQ_PROTOCOL_ERROR_ZMTP_INVALID_SEQUENCE);
            errno.set(ZError.EPROTO);
            return null;
        }
        cnPeerNonce = nonce;
        ...
        if (rc == 0) {
            ...
            session.getSocket().eventHandshakeFailedProtocol(session.getEndpoint(), ZMQUtilConstant.ZMQ_PROTOCOL_ERROR_ZMTP_CRYPTOGRAPHIC);
            errno.set(ZError.EPROTO);
            return null;
        }
    }
    ...
    private int processHello(Msg msg)
    {
        ...
        if (msg.size() != 200) {
            session.getSocket().eventHandshakeFailedProtocol(session.getEndpoint(), ZMQUtilConstant.ZMQ_PROTOCOL_ERROR_ZMTP_MALFORMED_COMMAND_HELLO);
            return ZError.EPROTO;
        }
        ...
        helloNonce.put("CurveZMQHELLO---".getBytes(ZMQUtilConstant.CHARSET));
    ...
    private int produceWelcome(Msg msg)
    {
        ...
        cookieNonce.put("COOKIE--".getBytes(ZMQUtilConstant.CHARSET));
        ...
        //  Create full nonce for encryption
        //  8-byte prefix plus 16-byte random nonce
        welcomeNonce.put("WELCOME-".getBytes(ZMQUtilConstant.CHARSET));
    ...
    private int processInitiate(Msg msg)
    {
        ...
        if (msg.size() < 257) {
            session.getSocket().eventHandshakeFailedProtocol(session.getEndpoint(), ZMQUtilConstant.ZMQ_PROTOCOL_ERROR_ZMTP_MALFORMED_COMMAND_INITIATE);
            return ZError.EPROTO;
        }
        ...
        initiateNonce.put("CurveZMQINITIATE".getBytes(ZMQUtilConstant.CHARSET));
        ...
        vouchNonce.put("VOUCH---".getBytes(ZMQUtilConstant.CHARSET));
        ...
        if (!compare(vouchPlaintext, cnClient, Curve.Size.ZERO.bytes(), 32)) {
            session.getSocket().eventHandshakeFailedProtocol(session.getEndpoint(), ZMQUtilConstant.ZMQ_PROTOCOL_ERROR_ZMTP_KEY_EXCHANGE);
            return ZError.EPROTO;
        }
    ...
    private int produceReady(Msg msg)
    {
        ...
        if (options.type == ZMQUtilConstant.ZMQ_REQ || options.type == ZMQUtilConstant.ZMQ_DEALER || options.type == ZMQUtilConstant.ZMQ_ROUTER) {
            addProperty(readyPlaintext, IDENTITY, options.identity);
        }

        int mlen = readyPlaintext.position();
        readyNonce.put("CurveZMQREADY---".getBytes(ZMQUtilConstant.CHARSET));
