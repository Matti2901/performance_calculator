...
public class Options
{
     ...
     * Default value for
     */
    public static final boolean DEFAULT_IPV6 = Options.PREFER_IPV6;
    /**
     * Default value for {@link ZMQUtilConstant#ZMQ_MSG_ALLOCATION_HEAP_THRESHOLD}
     */
    public static final int DEFAULT_ALLOCATION_HEAP_THRESHOLD = Config.MSG_ALLOCATION_HEAP_THRESHOLD.getValue();
    /**
     * Default value for {@link ZMQUtilConstant#ZMQ_MSG_ALLOCATOR}
     */
    public static final MsgAllocator DEFAULT_MSG_ALLOCATOR = new MsgAllocatorThreshold(DEFAULT_ALLOCATION_HEAP_THRESHOLD);
    //  High-water marks for message pipes.
    public int sendHwm = ZMQUtilConstant.DEFAULT_SEND_HWM;
    public int recvHwm = ZMQUtilConstant.DEFAULT_RECV_HWM;

    //  I/O thread affinity.
    public long affinity = ZMQUtilConstant.DEFAULT_AFFINITY;

    //  Socket identity
    public short  identitySize = (short) ZMQUtilConstant.DEFAULT_IDENTITY.length;
    public byte[] identity = ZMQUtilConstant.DEFAULT_IDENTITY;

    //  Maximum tranfer rate [kb/s]. Default 100kb/s.
    int rate = ZMQUtilConstant.DEFAULT_RATE;

    //  Reliability time interval [ms]. Default 10 seconds.
    int recoveryIvl = ZMQUtilConstant.DEFAULT_RECOVERY_IVL;

    // Sets the time-to-live field in every multicast packet sent.
    int multicastHops = ZMQUtilConstant.DEFAULT_MULTICAST_HOPS;

    // SO_SNDBUF and SO_RCVBUF to be passed to underlying transport sockets.
    public int sndbuf = ZMQUtilConstant.DEFAULT_SNDBUF;
    public int rcvbuf = ZMQUtilConstant.DEFAULT_RCVBUF;

    // Type of service (containing DSCP and ECN socket options)
    public int tos = ZMQUtilConstant.DEFAULT_TOS;

    //  Socket type.
    public int type = ZMQUtilConstant.DEFAULT_TYPE;

    //  Linger time, in milliseconds.
    public int linger = ZMQUtilConstant.DEFAULT_LINGER;

    //  Minimum interval between attempts to reconnect, in milliseconds.
    //  Default 100ms
    public int reconnectIvl = ZMQUtilConstant.DEFAULT_RECONNECT_IVL;
    //  Maximum interval between attempts to reconnect, in milliseconds.
    //  Default 0 (unused)
    public int reconnectIvlMax = ZMQUtilConstant.DEFAULT_RECONNECT_IVL_MAX;

    //  Maximum backlog for pending connections.
    public int backlog = ZMQUtilConstant.DEFAULT_BACKLOG;

    //  Maximal size of message to handle.
    public long maxMsgSize = ZMQUtilConstant.DEFAULT_MAX_MSG_SIZE;

    // The timeout for send/recv operations for this socket.
    int recvTimeout = ZMQUtilConstant.DEFAULT_RECV_TIMEOUT;
    int sendTimeout = ZMQUtilConstant.DEFAULT_SEND_TIMEOUT;

    //  If true, IPv6 is enabled (as well as IPv4)
    public boolean ipv6 = DEFAULT_IPV6;

    //  If false, connecting pipes are not attached immediately, meaning a send()
    //  on a socket with only connecting pipes would block
    public boolean immediate = ZMQUtilConstant.DEFAULT_IMMEDIATE;

    //  Addres of SOCKS proxy
    public String socksProxyAddress = ZMQUtilConstant.DEFAULT_SOCKS_PROXY_ADDRESS;

    //  TCP keep-alive settings.
    //  Defaults to -1 = do not change socket options
    public int tcpKeepAlive = ZMQUtilConstant.DEFAULT_TCP_KEEP_ALIVE;
    public int tcpKeepAliveCnt = ZMQUtilConstant.DEFAULT_TCP_KEEP_ALIVE_CNT;
    public int tcpKeepAliveIdle = ZMQUtilConstant.DEFAULT_TCP_KEEP_ALIVE_IDLE;
    public int tcpKeepAliveIntvl = ZMQUtilConstant.DEFAULT_TCP_KEEP_ALIVE_INTVL;

    /**
     * Default value for {@link ZMQUtilConstant#ZMQ_MECHANISM}
     */
    public static final Mechanisms DEFAULT_MECHANISM = Mechanisms.NULL;
    //  Security mechanism for all connections on this socket
    public Mechanisms mechanism = DEFAULT_MECHANISM;

    //  If peer is acting as server for PLAIN or CURVE mechanisms
    public boolean asServer = ZMQUtilConstant.DEFAULT_AS_SERVER;

    //  ZAP authentication domain
    public String zapDomain = ZMQUtilConstant.DEFAULT_ZAP_DOMAIN;
    ...
    boolean gssPlaintext = ZMQUtilConstant.DEFAULT_GSS_PLAINTEXT;
    ...
    //  Ignores hwm
    public boolean conflate = ZMQUtilConstant.DEFAULT_CONFLATE;
    ...
    public int handshakeIvl = ZMQUtilConstant.DEFAULT_HANDSHAKE_IVL;
    ...
    public int heartbeatTtl = ZMQUtilConstant.DEFAULT_HEARTBEAT_TTL;
    //  Time in milliseconds between sending heartbeat PING messages.
    public int heartbeatInterval = ZMQUtilConstant.DEFAULT_HEARTBEAT_INTERVAL;
    //  Time in milliseconds to wait for a PING response before disconnecting
    public int heartbeatTimeout = ZMQUtilConstant.DEFAULT_HEARTBEAT_TIMEOUT;
    // the ping context that will be sent with each ping message.
    public byte[] heartbeatContext = ZMQUtilConstant.DEFAULT_HEARTBEAT_CONTEXT;

    // threshold to allocate byte buffers on direct memory instead of heap.
    // Set to <= 0 to disable this system.
    public MsgAllocator allocator = DEFAULT_MSG_ALLOCATOR;

    /**
     * Default value for {@link ZMQUtilConstant#ZMQ_SELECTOR_PROVIDERCHOOSER}
     */
    public static final SelectorProviderChooser DEFAULT_SELECTOR_CHOOSER = null;
    public SelectorProviderChooser selectorChooser = DEFAULT_SELECTOR_CHOOSER;

    /**
     * Default value for {@link ZMQUtilConstant#ZMQ_HELLO_MSG}
     */
    public static final Msg DEFAULT_HELLO_MSG = null;
    // Hello msg to send to peer upon connecting
    public Msg helloMsg = DEFAULT_HELLO_MSG;
    public boolean canSendHelloMsg = false;

    /**
     * Default value for {@link ZMQUtilConstant#ZMQ_DISCONNECT_MSG}
     */
    public static final Msg DEFAULT_DISCONNECT_MSG = null;
    // Disconnect message to receive when peer disconnect
    public Msg disconnectMsg = DEFAULT_DISCONNECT_MSG;
    public boolean canReceiveDisconnectMsg = false;

    /**
     * Default value for {@link ZMQUtilConstant#ZMQ_HICCUP_MSG}
     */
    ...
    public Msg hiccupMsg = DEFAULT_HICCUP_MSG;
    public boolean canReceiveHiccupMsg = false;

    //  As Socket type on the network.
    public int asType = ZMQUtilConstant.DEFAULT_AS_TYPE;

    // A metadata record name where the self address will be stored if defined
    public String selfAddressPropertyName = ZMQUtilConstant.DEFAULT_SELF_ADDRESS_PROPERTY_NAME;
    ...
    public boolean setSocketOpt(int option, Object optval)
    {
        ...
        switch (option) {
        case ZMQUtilConstant.ZMQ_SNDHWM:
        ...
        case ZMQUtilConstant.ZMQ_RCVHWM:
        ...
        case ZMQUtilConstant.ZMQ_AFFINITY:
            affinity = (Long) optval;
            return true;

        case ZMQUtilConstant.ZMQ_IDENTITY:
        ...
        case ZMQUtilConstant.ZMQ_RATE:
            rate = (Integer) optval;
            return true;

        case ZMQUtilConstant.ZMQ_RECOVERY_IVL:
            recoveryIvl = (Integer) optval;
            return true;

        case ZMQUtilConstant.ZMQ_SNDBUF:
            sndbuf = (Integer) optval;
            return true;

        case ZMQUtilConstant.ZMQ_RCVBUF:
            rcvbuf = (Integer) optval;
            return true;

        case ZMQUtilConstant.ZMQ_TOS:
            tos = (Integer) optval;
            return true;

        case ZMQUtilConstant.ZMQ_LINGER:
            linger = (Integer) optval;
            return true;

        case ZMQUtilConstant.ZMQ_RECONNECT_IVL:
        ...
        case ZMQUtilConstant.ZMQ_RECONNECT_IVL_MAX:
        ...
        case ZMQUtilConstant.ZMQ_BACKLOG:
            backlog = (Integer) optval;
            return true;

        case ZMQUtilConstant.ZMQ_MAXMSGSIZE:
            maxMsgSize = (Long) optval;
            return true;

        case ZMQUtilConstant.ZMQ_MULTICAST_HOPS:
            multicastHops = (Integer) optval;
            return true;

        case ZMQUtilConstant.ZMQ_RCVTIMEO:
            recvTimeout = (Integer) optval;
            return true;

        case ZMQUtilConstant.ZMQ_SNDTIMEO:
            sendTimeout = (Integer) optval;
            return true;

        /*  Deprecated in favor of ZMQ_IPV6  */
        case ZMQUtilConstant.ZMQ_IPV4ONLY:
            return setSocketOpt(ZMQUtilConstant.ZMQ_IPV6, !parseBoolean(option, optval));

        /*  To replace the somewhat surprising IPV4ONLY */
        case ZMQUtilConstant.ZMQ_IPV6:
            ipv6 = parseBoolean(option, optval);
            return true;

        case ZMQUtilConstant.ZMQ_SOCKS_PROXY:
            socksProxyAddress = parseString(option, optval);
            return true;

        case ZMQUtilConstant.ZMQ_TCP_KEEPALIVE:
        ...
        case ZMQUtilConstant.ZMQ_TCP_KEEPALIVE_CNT:
            this.tcpKeepAliveCnt = ((Number) optval).intValue();
            return true;
        case ZMQUtilConstant.ZMQ_TCP_KEEPALIVE_IDLE:
            this.tcpKeepAliveIdle = ((Number) optval).intValue();
            return true;
        case ZMQUtilConstant.ZMQ_TCP_KEEPALIVE_INTVL:
            this.tcpKeepAliveIntvl = ((Number) optval).intValue();
            return true;

        case ZMQUtilConstant.ZMQ_IMMEDIATE:
            immediate = parseBoolean(option, optval);
            return true;

        case ZMQUtilConstant.ZMQ_DELAY_ATTACH_ON_CONNECT:
            immediate = !parseBoolean(option, optval);
            return true;

        case ZMQUtilConstant.ZMQ_TCP_ACCEPT_FILTER:
        ...
        case ZMQUtilConstant.ZMQ_PLAIN_PASSWORD:
            if (optval == null) {
                mechanism = Mechanisms.NULL;
                asServer = false;
                return true;
        ...
        case ZMQUtilConstant.ZMQ_ZAP_DOMAIN:
        ...
        case ZMQUtilConstant.ZMQ_CURVE_SERVER:
            asServer = parseBoolean(option, optval);
            mechanism = (asServer ? Mechanisms.CURVE : Mechanisms.NULL);
            return true;

        case ZMQUtilConstant.ZMQ_CURVE_PUBLICKEY:
            curvePublicKey = setCurveKey(option, optval, result);
            return result.get();

        case ZMQUtilConstant.ZMQ_CURVE_SECRETKEY:
            curveSecretKey = setCurveKey(option, optval, result);
            return result.get();

        case ZMQUtilConstant.ZMQ_CURVE_SERVERKEY:
        ...
        case ZMQUtilConstant.ZMQ_CONFLATE:
            conflate = parseBoolean(option, optval);
            return true;

        case ZMQUtilConstant.ZMQ_GSSAPI_SERVER:
            asServer = parseBoolean(option, optval);
            mechanism = Mechanisms.GSSAPI;
            return true;

        case ZMQUtilConstant.ZMQ_GSSAPI_PRINCIPAL:
            gssPrincipal = parseString(option, optval);
            mechanism = Mechanisms.GSSAPI;
            return true;

        case ZMQUtilConstant.ZMQ_GSSAPI_SERVICE_PRINCIPAL:
            gssServicePrincipal = parseString(option, optval);
            mechanism = Mechanisms.GSSAPI;
            return true;

        case ZMQUtilConstant.ZMQ_GSSAPI_PLAINTEXT:
            gssPlaintext = parseBoolean(option, optval);
            return true;

        case ZMQUtilConstant.ZMQ_HANDSHAKE_IVL:
        ...
        case ZMQUtilConstant.ZMQ_HEARTBEAT_IVL:
        ...
        case ZMQUtilConstant.ZMQ_HEARTBEAT_TIMEOUT:
        ...
        case ZMQUtilConstant.ZMQ_HEARTBEAT_TTL:
        ...
        case ZMQUtilConstant.ZMQ_HEARTBEAT_CONTEXT:
        ...
        case ZMQUtilConstant.ZMQ_DECODER:
        ...
        case ZMQUtilConstant.ZMQ_ENCODER:
        ...
        case ZMQUtilConstant.ZMQ_MSG_ALLOCATOR:
        ...
        case ZMQUtilConstant.ZMQ_HELLO_MSG:
            if (optval == null) {
                helloMsg = null;
            }
            else {
        ...
        case ZMQUtilConstant.ZMQ_DISCONNECT_MSG:
            if (optval == null) {
                disconnectMsg = null;
            }
            else {
        ...
        case ZMQUtilConstant.ZMQ_HICCUP_MSG:
            if (optval == null) {
                hiccupMsg = null;
            }
            else {
        ...
        case ZMQUtilConstant.ZMQ_AS_TYPE:
            this.asType = (Integer) optval;
            return true;

        case ZMQUtilConstant.ZMQ_SELFADDR_PROPERTY_NAME:
        ...
        if (optval == null) {
            ...
            if (optval instanceof byte[] && ((byte[]) optval).length == CURVE_KEYSIZE) {
                ...
                if (length == CURVE_KEYSIZE_Z85) {
                    ...
                    key = val.getBytes(ZMQUtilConstant.CHARSET);
                    result.set(true);
                    errno.set(0);
                }
                else {
    ...
    public Object getSocketOpt(int option)
    {
        ...
        case ZMQUtilConstant.ZMQ_MECHANISM:
            return mechanism;
        ...
        case ZMQUtilConstant.ZMQ_LAST_ENDPOINT:
            return lastEndpoint;
    ...
    public static String parseString(int option, Object optval)
    {
        if (optval instanceof String) {
            ...
            return new String((byte[]) optval, ZMQUtilConstant.CHARSET);
        }
    ...
    {
        if (optval instanceof String) {
            return ((String) optval).getBytes(ZMQUtilConstant.CHARSET);
        }
        else if (optval instanceof byte[]) {
            return (byte[]) optval;
        }
