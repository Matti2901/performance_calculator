...
public class CurveServerMechanism extends Mechanism
{
    ...
    private final byte[] cnClient = new byte[Size.PUBLICKEY.bytes()];
    ...
    public CurveServerMechanism(SessionBase session, Address peerAddress, Options options)
    {
        ...
        this.state = StateServer.EXPECT_HELLO;
        cnNonce = 1;
        cnPeerNonce = 1;
    ...
    public int nextHandshakeCommand(Msg msg)
    {
        ...
        case SEND_WELCOME:
            ...
            if (rc == 0) {
                state = StateServer.EXPECT_INITIATE;
            }
            break;
        case SEND_READY:
            rc = produceReady(msg);
            if (rc == 0) {
                state = StateServer.CONNECTED;
            }
            break;
        case SEND_ERROR:
            rc = produceError(msg);
            if (rc == 0) {
                state = StateServer.ERROR_SENT;
            }
            break;
        default:
            rc = ZError.EAGAIN;
    ...
    {
        assert (state == StateServer.CONNECTED);
    ...
    {
        if (state != StateServer.EXPECT_ZAP_REPLY) {
            return ZError.EFSM;
        }

        int rc = receiveAndProcessZapReply();
        if (rc == 0) {
            state = "200".equals(statusCode) ? StateServer.SEND_READY : StateServer.SEND_ERROR;
        }
        return rc;
    }
    ...
    {
        if (state == StateServer.CONNECTED) {
            return Status.READY;
        }
        else if (state == StateServer.ERROR_SENT) {
            return Status.ERROR;
        }
        else {
    ...
    private int processHello(Msg msg)
    {
        ...
        msg.getBytes(80, cnClient, 0, Size.PUBLICKEY.bytes());
        ...
        helloBox.position(Size.BOXZERO.bytes());
        ...
        if (rc != 0) {
            session.getSocket().eventHandshakeFailedProtocol(session.getEndpoint(), ZMQ.ZMQ_PROTOCOL_ERROR_ZMTP_CRYPTOGRAPHIC);
            state = StateServer.SEND_ERROR;
            statusCode = null;
            return 0;
        }

        state = StateServer.SEND_WELCOME;
        return 0;
    }

    private int produceWelcome(Msg msg)
    {
        ByteBuffer cookieNonce = ByteBuffer.allocate(Size.NONCE.bytes());
        ByteBuffer cookiePlaintext = ByteBuffer.allocate(Size.ZERO.bytes() + 64);
        ByteBuffer cookieCiphertext = ByteBuffer.allocate(Size.BOXZERO.bytes() + 80);
        ...
        cookiePlaintext.position(Size.ZERO.bytes());
        cookiePlaintext.put(cnClient);
        cookiePlaintext.put(cnSecret);

        //  Generate fresh cookie key
        cookieKey = cryptoBox.random(Size.KEY.bytes());
        ...
        ByteBuffer welcomeCiphertext = ByteBuffer.allocate(Size.BOXZERO.bytes() + 144);

        //  Create full nonce for encryption
        //  8-byte prefix plus 16-byte random nonce
        welcomeNonce.put("WELCOME-".getBytes(ZMQ.CHARSET));
        welcomeNonce.put(cryptoBox.random(Size.NONCE.bytes() - 8));
        ...
        cookieCiphertext.limit(Size.BOXZERO.bytes() + 80).position(Size.BOXZERO.bytes());
        ...
        msg.put(welcomeCiphertext, Size.BOXZERO.bytes(), 144);
    ...
    private int processInitiate(Msg msg)
    {
        ...
        ByteBuffer cookieNonce = ByteBuffer.allocate(Size.NONCE.bytes());
        ByteBuffer cookiePlaintext = ByteBuffer.allocate(Size.ZERO.bytes() + 64);
        ByteBuffer cookieBox = ByteBuffer.allocate(Size.BOXZERO.bytes() + 80);

        //  Open Box [C' + s'](t)
        cookieBox.position(Size.BOXZERO.bytes());
        ...
        if (!compare(cookiePlaintext, cnClient, Size.ZERO.bytes(), 32)
                || !compare(cookiePlaintext, cnSecret, Size.ZERO.bytes() + 32, 32)) {
            session.getSocket().eventHandshakeFailedProtocol(session.getEndpoint(), ZMQ.ZMQ_PROTOCOL_ERROR_ZMTP_CRYPTOGRAPHIC);
            return ZError.EPROTO;
        }

        int clen = msg.size() - 113 + Size.BOXZERO.bytes();
        ...
        initiateBox.position(Size.BOXZERO.bytes());
        msg.transfer(initiateBox, 113, clen - Size.BOXZERO.bytes());
        ...
        vouchBox.position(Size.BOXZERO.bytes());
        initiatePlaintext.limit(Size.ZERO.bytes() + 48 + 80).position(Size.ZERO.bytes() + 48);
        vouchBox.put(initiatePlaintext);

        vouchNonce.put("VOUCH---".getBytes(ZMQ.CHARSET));
        initiatePlaintext.limit(Size.ZERO.bytes() + 32 + 16).position(Size.ZERO.bytes() + 32);
        ...
        if (!compare(vouchPlaintext, cnClient, Size.ZERO.bytes(), 32)) {
        ...
        if (rc == 0) {
            ...
            if (rc == 0) {
                ...
                state = StateServer.EXPECT_ZAP_REPLY;
            }
            else {
                return -1;
            }
            ...
            state = StateServer.SEND_READY;
        }
        initiatePlaintext.position(0);
        initiatePlaintext.limit(clen);
        return parseMetadata(initiatePlaintext, Size.ZERO.bytes() + 128, false);
    }
    ...
    private int produceReady(Msg msg)
    {
        ...
        readyPlaintext.position(Size.ZERO.bytes());
        ...
        //  Box [metadata](S'->C')
        msg.put(readyBox, Size.BOXZERO.bytes(), mlen - Size.BOXZERO.bytes());

        assert (msg.size() == 14 + mlen - Size.BOXZERO.bytes());
        cnNonce++;
    ...
    private void sendZapRequest(byte[] key)
    {
        ...
        //  Credentials frame
        Msg msg = new Msg(Size.PUBLICKEY.bytes());
        msg.put(key, 0, Size.PUBLICKEY.bytes());
