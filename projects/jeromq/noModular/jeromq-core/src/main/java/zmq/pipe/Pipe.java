...
import zmq.Config;
import zmq.core.Msg;
import zmq.core.ZObject;
import zmq.util.blob.Blob;
...
public class Pipe extends ZObject
{
    ...
    public boolean checkRead()
    {
        ...
        if (state != PipeState.ACTIVE && state != PipeState.WAITING_FOR_DELIMITER) {
            return false;
        }
    ...
    {
        if (!outActive || state != PipeState.ACTIVE) {
            return false;
        }
    ...
    public void flush()
    {
        ...
        if (state == PipeState.TERM_ACK_SENT) {
            return;
        }
    ...
    {
        if (!inActive && (state == PipeState.ACTIVE || state == PipeState.WAITING_FOR_DELIMITER)) {
            inActive = true;
    ...
    {
        assert (state == PipeState.ACTIVE || state == PipeState.DELIMITER_RECEIVED || state == PipeState.TERM_REQ_SENT_1);
        ...
        if (state == PipeState.ACTIVE) {
            if (delay) {
                state = PipeState.WAITING_FOR_DELIMITER;
            }
            else {
                state = PipeState.TERM_ACK_SENT;
                outpipe = null;
        ...
        if (state == PipeState.DELIMITER_RECEIVED) {
            state = PipeState.TERM_ACK_SENT;
            outpipe = null;
            sendPipeTermAck(peer);
        }
        ...
        //  own ack.
        if (state == PipeState.TERM_REQ_SENT_1) {
            state = PipeState.TERM_REQ_SENT_2;
            outpipe = null;
            sendPipeTermAck(peer);
        }
    }
    ...
    protected void processPipeTermAck()
    {
        ...
        if (state == PipeState.TERM_REQ_SENT_1) {
            ...
            assert (state == PipeState.TERM_ACK_SENT || state == PipeState.TERM_REQ_SENT_2);
        }
    ...
    public void terminate(boolean delay)
    {
        ...
        if (state == PipeState.TERM_REQ_SENT_1 || state == PipeState.TERM_REQ_SENT_2) {
            return;
        }
        //  If the pipe is in the final phase of async termination, it's going to
        //  closed anyway. No need to do anything special here.
        else if (state == PipeState.TERM_ACK_SENT) {
            return;
        }
        //  The simple sync termination case. Ask the peer to terminate and wait
        //  for the ack.
        else if (state == PipeState.ACTIVE) {
            sendPipeTerm(peer);
            state = PipeState.TERM_REQ_SENT_1;
        }
        //  There are still pending messages available, but the user calls
        //  'terminate'. We can act as if all the pending messages were read.
        else if (state == PipeState.WAITING_FOR_DELIMITER && !this.delay) {
            outpipe = null;
            sendPipeTermAck(peer);
            state = PipeState.TERM_ACK_SENT;
        }
        //  If there are pending messages still available, do nothing.
        else if (state == PipeState.WAITING_FOR_DELIMITER) {
            // do nothing
        }
        //  We've already got delimiter, but not term command yet. We can ignore
        //  the delimiter and ack synchronously terminate as if we were in
        //  active state.
        else if (state == PipeState.DELIMITER_RECEIVED) {
            sendPipeTerm(peer);
            state = PipeState.TERM_REQ_SENT_1;
        }
    ...
    {
        assert (state == PipeState.ACTIVE || state == PipeState.WAITING_FOR_DELIMITER);

        if (state == PipeState.ACTIVE) {
            state = PipeState.DELIMITER_RECEIVED;
        }
        else {
            outpipe = null;
            sendPipeTermAck(peer);
