...
public class StreamEngine implements IEngine, IPollEvents
{
    ...
    public void inEvent()
    {
        ...
        while (insize > 0) {
            ...
            Result result = decoder.decode(inpos, insize, processed);
            assert (processed.get() <= insize);
            insize -= processed.get();

            if (result == Result.MORE_DATA) {
                rc = true;
                break;
            }
    ...
    private Msg nextHandshakeCommand()
    {
        ...
        if (mechanism.status() == Status.READY) {
            mechanismReady();

            return pullAndEncode.get();
        }
        else if (mechanism.status() == Status.ERROR) {
    ...
    private void error(ErrorReason error)
    {
        ...
        session.engineError(!handshaking && (mechanism == null ||
                mechanism.status() != Status.HANDSHAKING), error);
        unplug();
        destroy();
    }
    ...
    Msg producePongMessage(byte[] pingContext)
    {
        assert (mechanism != null);
    ...
    private boolean processHeartbeatMessage(Msg msg)
    {
        ...
        nextMsg = new ProducePongMessage(this, pingContext);
        outEvent();
