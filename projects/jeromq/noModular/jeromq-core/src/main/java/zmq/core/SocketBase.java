...
import zmq.core.socket.SocketEventHandler;
...
import zmq.util.map.MultiMap;

public abstract class SocketBase extends Own implements IPollEvents, IPipeEvents
{
    ...
    private final AtomicReference<EventConsummer> monitor;
     ...
     *         registered and consumer is not null.
     */
    public final boolean setEventHook(EventConsummer consumer, int events)
    {
        synchronized (monitor) {
            if (ctxTerminated.get()) {
                errno.set(ZError.ETERM);
    ...
    private void event(String addr, Object arg, int event)
    {
        synchronized (monitor) {
            ...
            monitorEvent(new Event(event, addr, arg));
        }
    }

    //  Send a monitor event
    protected final void monitorEvent(Event event)
    {
        if (monitor.get() == null) {
            return;
        }
    ...
    private void stopMonitor()
    {
        ...
        if (monitor.get() != null) {
            if ((monitorEvents & ZMQ.ZMQ_EVENT_MONITOR_STOPPED) != 0) {
                monitorEvent(new Event(ZMQ.ZMQ_EVENT_MONITOR_STOPPED, "", 0));
            }
            monitor.get().close();
            monitor.set(null);
            monitorEvents = 0;
