...
import zmq.core.type.StateProxy;
...
class Proxy
{
    ...
    private boolean start(SocketBase frontend, SocketBase backend, SocketBase capture, SocketBase control)
    {
        ...
        try {
            while (state != StateProxy.TERMINATED) {
    ...
    {
        return state == StateProxy.ACTIVE && read.isReadable() && (frontend == backend || write.isWritable());
    }
