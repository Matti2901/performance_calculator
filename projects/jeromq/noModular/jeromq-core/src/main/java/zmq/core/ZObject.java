...
public abstract class ZObject
{
    ...
    protected final void processCommand(Command cmd)
    {
    ...
    {
        destination.incSeqnum();
        Command cmd = new Command(destination, CommandType.OWN, object);
        sendCommand(cmd);
    }
    ...
    {
        Command cmd = new Command(destination, CommandType.ACTIVATE_READ);
        sendCommand(cmd);
    }

    protected final void sendActivateWrite(Pipe destination, long msgsRead)
    {
        Command cmd = new Command(destination, CommandType.ACTIVATE_WRITE, msgsRead);
        sendCommand(cmd);
    }
    ...
    {
        Command cmd = new Command(destination, CommandType.PIPE_TERM);
        sendCommand(cmd);
    }

    protected final void sendPipeTermAck(Pipe destination)
    {
        Command cmd = new Command(destination, CommandType.PIPE_TERM_ACK);
        sendCommand(cmd);
    }

    protected final void sendTermReq(Own destination, Own object)
    {
        Command cmd = new Command(destination, CommandType.TERM_REQ, object);
        sendCommand(cmd);
    }
    ...
    {
        Command cmd = new Command(destination, CommandType.TERM_ACK);
        sendCommand(cmd);
    }

    protected final void sendReap(SocketBase socket)
    {
        Command cmd = new Command(ctx.getReaper(), CommandType.REAP, socket);
        sendCommand(cmd);
    }

    protected final void sendReapAck()
    {
        Command cmd = new Command(this, CommandType.REAP_ACK);
        sendCommand(cmd);
    }

    protected final void sendReaped()
    {
        Command cmd = new Command(ctx.getReaper(), CommandType.REAPED);
        sendCommand(cmd);
    }

    protected final void sendInprocConnected(SocketBase socket)
    {
        Command cmd = new Command(socket, CommandType.INPROC_CONNECTED);
        sendCommand(cmd);
    }
    ...
    {
        Command cmd = new Command(this, CommandType.CANCEL);
        sendCommand(cmd);
    }
