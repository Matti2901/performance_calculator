...
import zmq.socket.peer.Peer;
import zmq.socket.peer.Raw;
...
import zmq.socket.radiodish.dish.DishSession;
import zmq.socket.radiodish.Radio;
import zmq.socket.radiodish.radiosession.RadioSession;
import zmq.socket.reqrep.dealer.Dealer;
import zmq.socket.reqrep.dealer.session.ReqSession;
import zmq.socket.reqrep.router.Rep;
import zmq.socket.reqrep.dealer.Req;
import zmq.socket.reqrep.router.Router;
...
import zmq.socket.stream.Stream;
...
public enum Sockets
{
    ...
    REQ("REP", "ROUTER") {
        ...
        {
            return new ReqSession(ioThread, connect, socket, options, addr);
        }
    },
    REP("REQ", "DEALER") {
        @Override
    ...
    RADIO("DISH") {
        ...
        public SessionBase create(IOThread ioThread, boolean connect, SocketBase socket, Options options, Address addr)
        {
            return new RadioSession(ioThread, connect, socket, options, addr);
        }
    },
    DISH("RADIO") {
        @Override
        ...
        public SessionBase create(IOThread ioThread, boolean connect, SocketBase socket, Options options, Address addr)
        {
            return new DishSession(ioThread, connect, socket, options, addr);
        }
    },
    CHANNEL("CHANNEL") {
        @Override
