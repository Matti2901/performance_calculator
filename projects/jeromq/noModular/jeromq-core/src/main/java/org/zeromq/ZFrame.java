...
public class ZFrame
{
    ...
    public boolean send(Socket socket, int flags)
    {
        ...
        final zmq.core.Msg msg = new Msg(data);

        if (group != null) {
            msg.setGroup(group);
        }
        int sendFlags = (flags & ZFrame.MORE) == ZFrame.MORE ? zmq.core.ZMQ.ZMQ_SNDMORE : 0;
        sendFlags |= (flags & ZFrame.DONTWAIT) == ZFrame.DONTWAIT ? zmq.core.ZMQ.ZMQ_DONTWAIT : 0;
