 ...
 *    <ul>
 *      <li>with built-in low-level {@link org.zeromq.ZProxy.ZmqPump} (zmq.core.ZMQ): useful for performances
    ...
    private static final class ZmqPump implements Pump
    {
        ...
        public boolean flow(Plug splug, Socket source, Socket capture, Plug dplug, Socket destination)
        {
            ...
            while (true) {
                ...
                long more = src.getSocketOpt(zmq.core.ZMQ.ZMQ_RCVMORE);
                ...
                if (cpt != null) {
                    ...
                    rc = cpt.send(ctrl, more > 0 ? zmq.core.ZMQ.ZMQ_SNDMORE : 0);
                    if (!rc) {
                        // not successful, but we can still try to send it to the destination
                    }
                }
                ...
                // we send the packet
                rc = dst.send(msg, more > 0 ? zmq.core.ZMQ.ZMQ_SNDMORE : 0);
