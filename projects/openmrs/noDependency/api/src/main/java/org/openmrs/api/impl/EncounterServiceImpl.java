	...
	public Encounter saveEncounter(Encounter encounter) throws APIException {
		...
		if (!isNewEncounter) {
			...
			for (Obs obs : encounter.getAllFlattenedObs(true)) {
				...
				if (Comparator.compare(originalDate, newDate) != 0
				        && Comparator.compare(obs.getObsDatetime(), originalDate) == 0) {
				...
				if (!Comparator.nullSafeEquals(newLocation, originalLocation) && obs.getLocation().equals(originalLocation)) {
