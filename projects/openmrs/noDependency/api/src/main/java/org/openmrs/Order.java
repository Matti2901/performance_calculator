...
import org.openmrs.stable.exception.OrderException;
import org.openmrs.util.manager.HibernateUtil;
import org.openmrs.stable.OrderUtil;
...
public class Order extends BaseCustomizableData<OrderAttribute> implements FormRecordable {
	...
	public boolean isActivated(Date checkDate) {
		...
		return Comparator.compare(dateActivated, checkDate) <= 0;
	}
	...
	public boolean isDiscontinued(Date aCheckDate) {
		if (dateStopped != null && autoExpireDate != null && dateStopped.after(autoExpireDate)) {
			throw new OrderException("Order.error.invalidDateStoppedAndAutoExpireDate", (Object[]) null);
		}
		if (getVoided()) {
			return false;
		}
	...
	public boolean hasSameOrderableAs(Order otherOrder) {
		...
		return Comparator.nullSafeEquals(this.getConcept(), otherOrder.getConcept());
	}
