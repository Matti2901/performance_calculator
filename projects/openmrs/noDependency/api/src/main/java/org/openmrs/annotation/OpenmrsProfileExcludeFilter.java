...
import org.openmrs.module.ModulClassUtil;
...
public class OpenmrsProfileExcludeFilter implements TypeFilter {
	...
	public boolean matchOpenmrsProfileAttributes(Map<String, Object> openmrsProfile) {
		...
		if (StringUtils.isNotBlank((String) openmrsPlatformVersion)
				&& !ModulClassUtil.matchRequiredVersions(OpenmrsConstants.OPENMRS_VERSION_SHORT, (String) openmrsPlatformVersion)) {
			return false;
		}
			...
			if ("!".equals(moduleAndVersion.substring(0, 1))) {
				...
				for (Module module : ModuleFactory.getStartedModules()) {
					if (module.getModuleId().equals(moduleId)
							&& ModulClassUtil.matchRequiredVersions(module.getVersion(), moduleVersion)) {
						moduleMatched = true;
						break;
					}
				}
