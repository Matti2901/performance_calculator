...
public class AllergyReaction extends BaseOpenmrsObject implements java.io.Serializable{
	...
	public boolean hasSameValues(AllergyReaction reaction) {
		if (!Comparator.nullSafeEquals(getAllergyReactionId(), reaction.getAllergyReactionId())) {
			return false;
		}
		if (!Comparator.nullSafeEquals(getReaction(), reaction.getReaction())) {
			...
			if (getReaction() != null && reaction.getReaction() != null) {
				if (!Comparator.nullSafeEquals(getReaction().getConceptId(), reaction.getReaction().getConceptId())) {
					return false;
				}
			}
			else {
		...
		return Comparator.nullSafeEquals(getReactionNonCoded(), reaction.getReactionNonCoded());
	}
}
